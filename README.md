...
**[v1.19.11](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.11) · 创建百炼 Agent 报 400 · 错误原因不再被吞掉**
点「在百炼上创建 Agent」失败,提示是「upstream 400 —— 先确认业务空间 ID、MCP 服务 ID 都对」,**而那两个 ID 本来就是对的**。真正的原因百炼说得很清楚,却被本应用收到后丢掉了。修好显示之后暴露出请求体两处形状错(`model` 应是对象不是字符串;MCP 类型应是 `customer` 而非 `custom`),创建与更新两条路径各有一份、一起改。现在百炼的原话放在最前面,我们的猜测降到后面并注明是猜测,同时落日志。顺带:五步向导最后一步要点的按钮原来是个 10px 透明文字链接、还在向导外面,已改成主按钮并放进第五步。**无 DB 迁移。**

**[v1.19.9](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.9) · 修复百炼托管接入连不上(MCP 握手没做版本协商)**
按教程配好百炼托管 Agent 后连接直接失败(`Unsupported protocol version from the server: 2025-06-18`),**卡在握手第一步**,连工具列表都拿不到。原因是 MCP 要求双方**协商**版本 —— 客户端支持哪个、服务端也支持就必须回同一个,而我们不管对方说什么一律回自己的版本(代码注释里那句「照回自己的,由它决定要不要继续」本身就是对规范的误读)。现在支持三个版本,要哪个回哪个;顺带修了客户端握手不带参数时会 500 的边缘情况。**配置不用改**,升级后在百炼控制台重试即可。**无 DB 迁移。**
