...
**[v1.19.5](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.5) · 刚上传的截图能删也能点开看了**
「截图导入持仓」里上传错的图删不掉、传完点开也没反应。两件事同一个原因:上传页有**两个缩略图区**,刚上传的那批由前端画,没有 ✕ 也不能点开;删除和放大只挂在刷新后由服务端渲染的那一批上 —— 功能一直"是有的",只是出现在你不需要它的时刻。现在每张右上角都有 ✕,点缩略图能放大,**图还在传的过程中就能点开确认**;删一张之后计数、预估费用、「开始识别」的可用状态都跟着退回去。顺带修了「传完图刷新一下『开始识别』会变灰」。**无 DB 迁移。**
