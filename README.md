...
**[v1.21.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.21.0) · 自定义支出分类:记一笔时点一下,或者把账单整月导进来**
支出原来只有四个「性质」值,看不出餐饮占多少、交通占多少。现在三条路各走各的:**填一个总数**(不变,没有分类)/ **逐笔录入**(记一笔时点一下图标宫格,一次点击)/ **账单导入**(传支付宝微信月账单 csv,几百笔自动归类)。导入是这版最值钱的部分:归类**三层兜底**(渠道分类 → 你改过并记住的规则 → AI,**只送商户名不送金额**),每行标出归类**依据**、没把握的排最前,**账户也是逐笔推荐的**(一份账单里余额宝/花呗/银行卡都有,算到一个账户上会让几个账户的余额和收益率一起错),剔除与跳过的笔数都显式列出。「落到账户」可选 —— 钱已经从余额扣过了就只记花在哪。**不建类目的话页面逐字不变。含 DB 迁移 V59**(纯新增)。
