...
**[v1.18.5](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.5) · 手填余额不再被估值抹掉 · 贵金属补进资产体检**
挂着持仓的账户(基金/股票/加密/贵金属)余额由**持仓合计**算出,于是你在填报页**直接敲的余额**会被下一次自动估值抹掉 —— 真实时间线:`14:42 手填 451,497.63` → `16:10 估值写回 375,248.71`,76,248.92 又没了。这是同一问题的**第三个变种**,被前两次修复从中间漏过去(v1.18.1 只管划转/收入、v1.18.3 的写回拦截只认**流水**,而手填两者都不是)。现在:你敲的 X 与(持仓+现金)的差额会**记成一行现金**,下次估值重算 = X,数字站得住,差额在持仓页看得见可改。
同版还修:资产体检的「投资类账户」判据漏了 v0.14 加的 **METAL** —— 贵金属账户被「持有期/收益/回撤」三条规则**静默跳过**。三条语义收成 `AccountType` 上的具名谓词,并加**结构性测试**:以后加新类型不表态就直接红。模型型号失效时会**点名型号**并给出下一步(带日期的指向不带日期的 `doubao-seed-evolving`)。**无 DB 迁移。**

**[v1.18.4](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.4) · 关掉的功能不再拦住保存 · 火山方舟预置推荐型号**
配好主选模型、**取消勾选**「启用持仓截图导入」,保存却报 `截图识别:请选择平台` —— 一个已经关掉的功能凭什么还要求你填。根因是三组走同一套校验、一律要求平台选得出来;只配了 DeepSeek(无视觉能力)的人更是死路。现在按「你想干什么」分支:关掉 → 那一组一个字不校验;想开但没有有视觉能力的平台 → 勾选框**直接禁用**并写明去配哪家;选了没配密钥的平台 → 当面拒绝。
火山方舟以前只给一句「这一家没有可预置的型号」—— 重新调研后那个前提已不成立(方舟支持**直接填 Model ID**):型号下拉已预置推荐型号,默认取**不带日期**的 `doubao-seed-evolving`,并给出控制台直达链接。**无 DB 迁移。**
