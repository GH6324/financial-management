...
**[v1.21.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.21.2) · 批量导入不再干等 AI · 五个分桶都能点开核对**
传完账单页面停在「读到 428 笔」几十秒不动 —— 那是在同步等 AI 归类。现在**上传阶段根本不调 AI**,确认页秒开;AI 变成页面上一个按钮,只有它自己转圈,不点也能提交,**你手改过的行 AI 永不覆盖**。分桶从一串数字变成**可点的页签**(要导入 / 性质另算 / 收入 / 已剔除 / 已存在),剔错了能「捞回来」;「已存在」故意不给强制再导 —— 那等于给一个制造双份的按钮。流水列表每页默认 **10 条**(可切 10/20/50/100)并新增**筛选器**(类目 / 分类 / 账户 / 成员,只列本月实际有的值)。顺带修掉报表「支出构成」逐笔抽屉从 v1.21.0 起一直为空的问题。**无 DB 迁移。**

**[v1.21.1](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.21.1) · 流水列表能搜能翻页了**
v1.21.0 让一次导入能落进几百笔,而填报页的流水列表还是全量平铺的。现在加了**搜索 + 分页**(可搜类目/消费分类/账户/备注/主理人/金额/日期,空格分隔 = 全部命中;合计行永远是**全部**的合计,不随筛选变);列表**显示消费分类**(原来只显示「日常开支」,导入的几百笔看着一模一样);日期改成**交易日期**(原来是填报时间,导入的几百笔全是同一时刻,按日期搜直接全中)。「从账单导入」改名**「批量导入」**。**无 DB 迁移。**
