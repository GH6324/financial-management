...
**v1.24(在研)· 这个月为什么多花了,砍不砍得掉**

支出那一章从「钱花在哪」扩成一整章分析:比上期的**归因瀑布**(哪个类目涨了多少)、
**刚性 / 弹性 / 一次性**三分、以及剔掉一次性支出之后的**常态月均**。
原来两个都叫「钱花在哪了」、数还对不上的区块合并成一块两层结构。

**[v1.23.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.23.0) · 账期什么时候关,交给你定**
系统原来在次月 1 号凌晨 `00:30` 就把上月关了,而微信 / 支付宝 / 信用卡最方便拿到的是「整月支出」—— 那个数要等月底过完才有。于是「月初拿完整账单再填」这条最自然的路径,反而得先去管理页重开账期。现在关账宽限可配(**立即 / T+2 / T+5 / 我自己关**,默认立即,与旧版逐分一致),宽限期内**上月与本月同时可填**:填报页两期并排、批量导入默认记到上月、报表锚上月而不倒退。顺带修了三处静默算错(月均支出把半填期算了进去、上月宽限只剩一天却一条提醒都不发、报表在宽限里退回上上期)。**含 DB 迁移 V61(只加列)。**

**[v1.22.4](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.22.4) · 首屏:流水列表不再裸奔,目录不再迟到**
进填报页时流水列表会**先铺开一条望不到头的长龙**,过几秒才缩成分页 —— 实测裸奔 **2906ms**。根因是前端分页的前提就是把整月几百行全量渲染进 DOM,而分页脚本要等 `DOMContentLoaded`;改成让 CSS(在 head 里、阻塞渲染,比任何脚本都早)先藏掉多余的行,脚本接管时摘掉 → **0ms**。长文目录要等一会儿才显示,根因不是加载慢而是 **DOM 顺序**:它排在主内容之后、靠 `order:-1` 视觉上放左列,而 **`order` 改得了绘制位置、改不了字节到达的先后**;移到 DOM 最前 → 与主内容同时到达。**无 DB 迁移。**
