...
**[v1.19.8](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.8) · 审计日志修正:11 条记录指错了实体**
排查一个用户报的问题时,按持仓 ID 查审计日志,查到的记录其实讲的是另一个东西 —— 代码把「账户 ID」写进了标着「持仓」的字段里,两者编号恰好相同,于是排查被引向完全错误的方向。全仓扫下来 **11 处**同一形状(类型写「快照/流水/持仓/转账」、ID 传的全是账户 ID),说明是当初约定没讲清而非手滑。这两个字段是审计日志**唯一能被程序化检索的部分**,指错实体比没有更糟。已统一并加自动检查;历史记录不动,审计页不显示该字段,**对你的数据和界面零影响**。

**[v1.19.7](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.7) · 管理页补上漏掉的入口 · 百炼接入教程重写**
「AI 接入」和「账目对账」一直只挂在管理子页的侧边栏,而点「管理」看到的落地页卡片里没有 —— 功能一直在,只是够不着。同类问题此前发生过一次,当时的检查只认那一个页面的路径,新页面漏了也不报;这一版改成**集合比对**(侧边栏有的落地页必须也有,漏了直接点名路径)。百炼接入教程重写了最关键的一段:控制台点「创建 MCP 服务」后**还有一个四选一**,原教程漏了它 —— 选错(如「插件」)会被要求逐个填工具描述,而那完全没必要。现在会告诉你选「使用脚本部署」、另外三个各是什么、`type` 与地址末尾必须配对(错配报 404 不是鉴权错)、`Authorization` 的值怎么写,并附一条 curl 自测命令。**无 DB 迁移。**
