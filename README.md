...
### [v1.6.23 · 券商对接入口从 `⋯` 移回行内 + 提前还贷决策器补上入口](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.23)

一位用户报「券商类账户那套自动化对接(富途 / 老虎)的入口是不是丢了」。查证:**功能一行没丢** —— 是上一批 UI 优化按「行内按钮太多」,把账户表格里那个可见的「券商」按钮和「归档 / 导出 CSV / 划转」一起收进了没有文字的 `⋯` 菜单。**但对用户来说「还在但找不到」和「没了」没有区别**,所以这个报障是对的。现在券商按钮回到行内(只在股票 / 加密 / 贵金属账户出现),低频维护动作继续留在 `⋯` 里 —— 错在把两类东西按数量混着收纳:**能力入口**是这一行通往别处的门,**维护动作**是对这一行本身的操作。
顺手排查还发现一个更久的问题:**提前还贷决策器此前全站零入口** —— `/reports/refinance` 从 v0.4 就在、README 与落地页都在宣传,但没有任何地方链接过去,只能手敲 URL;而首页洞察条会提示「可考虑加速偿还」却不给去处。现在报表页负债段加了「要不要提前还贷?打开决策器」,首页那枚提示也变成可点直达。
**为什么原来没被发现**:当时那条守护就叫「券商入口在账户页」,却**一直是绿的** —— 它检查的是"模板里有没有这个链接",而链接确实还在,只是被埋进了折叠菜单。所以这次补的是机制:**功能入口登记表**(能力→入口页→期望可见层级)+ **运行时可见性检查**(电脑 1440×900 与手机 390×844 渲染真实页面,断言入口有面积、点击能命中它自己、不在未展开的折叠容器里)。守护有效性验证过:把问题放回去,两条守护立刻转红。

### [v1.6.22 · 数据卷老密码自愈(不删数据)+ 三处说谎的判据改真实查询](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.22)

上一版修好了大陆拉镜像,那位用户却换了个地方卡住:应用一直刷 `Access denied for user 'finance'` 无限重启,而脚本只说「90s 没就绪,常见:DB 还在初始化 / 端口被占」—— **两个猜测都不对**。
真因:MySQL 的账号密码**只在第一次初始化数据卷时**写入,而 Docker 的**命名卷不随仓库目录一起消失** —— 他为拿上一版修复重新下载了仓库,`.env` 换了新随机密码,卷里还是老密码。**这个坎是我们自己的升级指引造成的。**
更糟的是三处判据同时说谎(数据库健康检查报 Healthy、容器入口报「MySQL 就绪」、空库探测报「表存在数=1」),而且不是三个独立 bug —— 是同一个原语被误用了三次:`mysqladmin ping` 在密码错误时**也返回成功**(它问的是"服务器活着吗",不是"我能用吗")。
现在:脚本主动验账号,进不去就用 MySQL 官方的密码重置手法把新密码同步进已有数据库,**不需要旧密码、业务数据一行不动**,修完自动继续;所有判据改成真实查询,密码不对 11 秒就准确归因(原先白等 120 秒还谎报就绪);同步失败就**停在原地不动数据**,并写明 `down -v` 会删掉全部数据、不可恢复。**正卡在这个报错上的话:不需要删数据,`git pull` 后重跑 `bash deploy/docker-up.sh` 即可。**
...
| 测试 | JUnit 5 · 431 单元(含 AttributionEngine 归因两步法闭合 + RebalancePlan 核销规则 + PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 511 黑盒回归 |
