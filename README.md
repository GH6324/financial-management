...
**[v1.19.4](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.4) · 截图识别失败不再伪装成「所有持仓都卖了」**
线上事故:上传持仓截图后「完全失败」,页面却提示「没有任何持仓变动」。根因不在识别 —— 是视觉模型额度用完、上游拒绝调用,而**这个失败被吞成了成功**:一张都没识别出来 → 系统当成「这些持仓这次都没截到」→ 把账户里**每条持仓都标成「卖出?」**,页面上没有一个字提到失败过。卖出项默认「保留」才没出事,勾一下归档就会清空。
现在:全失败进独立失败态,**不生成比对表、页面上没有确认按钮**(误确认物理上做不到);部分图失败则整轮不提示卖出(「没识别出来」≠「卖掉了」);失败原因分清额度/密钥/限流/型号并指向该去哪处理 —— 以前一律「请重试」,而额度用完时那是错的建议。**无 DB 迁移。**
