...
### [v1.6.19 · 环图不再压图例 · 默认看板成员结构 · 切片排行 Top5](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.19)

**电脑端「资产配置」环图压住右侧图例**:那张卡在电脑上只有 386px 宽(窗口 1024 时仅 229px),右侧图例吃掉 181px,而环半径是**写死的**(上一版明确要求"两个饼图不能一大一小",不能退回自适应)→ 环右缘 221 越过图例左缘 193、压进 28px;「按成员分布」卡宽 1024 所以看不出。两卡宽度差 4 倍,右侧图例没法都安全 → 三步:图例一律移**下方** + 电脑端半径 118→104 + **图例文案缩短**(窗口 1024 时的决定性一步:环上已标金额的扇片图例不再重复金额,名称也去掉「(WEALTH)」英文码,与环上标签同一套写法)。实测 1440/1280/1024/390 四档零重叠,两图半径仍相等。
**旭日下钻**默认看板改为「成员结构」、下一层默认「平台」,并挪到看板列表第一位(默认项不在最左会让人以为选错了 —— 看板横滑,第三个未必在视野内)。**切片排行**默认只出前 5 条,点「展示全部(还有 N 项)」原地展开;它承担**完整列表**角色(旭日合并过小扇片时指路到这儿),所以只折起不截断、展开不重新请求。
