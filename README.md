...
**[v1.9.4](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.9.4) · 财富水位不再骗你说「需要至少 2 期」**
关了三期却一直显示「需要至少 2 期净资产数据 + 宏观基准」—— 两个条件都不是真的。给财富水位用的序列每期都减掉「本期首次出现账户的净值」,**包括窗口首期**,而首期的首次出现账户按定义是全部账户 → **首点恒为 0**,而水位以首点为锚、锚≤0 判不可用。只要时间范围含家庭首期,这一节就永远不出现(新用户从来没见过;老用户**新加一个账户**后短窗口也会被打死)。
修法:首期那笔存量本金是起跑线不是「注入」,不减;第二期起照旧剔除。已发布 tag 建零差异基线验证:**已能显示的时间范围数字逐字不变**。兜底文案也按原因分开说了真话。**无 DB 迁移。**
