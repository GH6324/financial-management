...
**[v1.24.3](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.24.3) · 超级 Agent 答案里的数字回来了**
这个功能的设计是「模型只写引用标记、碰不到数字本身」,而托管模式下工具在百炼那边执行、引用产在另一个请求里 —— 中间断了,于是模型老老实实写了标记,库里一行引用都没有:用户看到的是「全家现在的净资产是这个数:」**后面什么都没有**。同一轮还修掉三个只有从真机页面上才测得出来的问题:SSE 被反向代理缓冲 + 掐断(稍复杂的问题直接 504,**应用侧已自带防护,不需要你改 nginx**)、多次工具调用的引用 key 撞车导致**挂错数字**、图表少一个右花括号就整张静默消失。另外 AI 主备全挂时管理页现在会明说 —— 之前挂了 20 天没人知道,因为每一层都优雅降级了。**无 DB 迁移。**
...
| 测试 | JUnit 5 · **927 单元** / 119 e2e 断言(3 条浏览器主线,真浏览器 + 真 DB)/ **876 黑盒回归**护栏 —— 每条护栏守的是哪个坑,见 [QA case 库](docs/qa-cases.md) |
