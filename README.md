...
- **v1.21 设计文档(在研 · 待评审)**:[`prd/v1.21.md`](prd/v1.21.md) · [`tech-design/v1.21.md`](tech-design/v1.21.md) · [`preview/v1.21/expense-categories.html`](preview/v1.21/expense-categories.html)(源自 [issue #18](https://github.com/LuoDi-Nate/financial-management/issues/18),拆自 #17 第 3 条 —— 自定义支出分类。与「刻意不做消费品类细化」的冲突化解在结构上:**逐笔归 APP,分类归我们** —— 流水留在支付宝/微信/银行,月底只进「按类目的汇总数」;**分类是月度总额的「展开」而非第三种记账方式**,合计同事务写回 PMC,家庭支出所有口径(储蓄率/月均/紧急储备/FIRE)零改动。类目是**一棵两层树的两个深度**(简单版/复杂版起步包,切换兼容靠父子边,历史无损);进数给三条通道:手抄 / 支付宝·微信账单文件导入(逐渠道档案:交互路径/zip密码/编码/列结构)/ AI 截图导入(qwen-vl 只转写月度统计页的类目名+金额,不做数学);手填与各渠道导入按**来源行**共处,重导只替换该渠道、手工修正永不被冲掉)
