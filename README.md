...
### [v1.1.7 · 指标多选 + 看板重排 + 打标直达入口 + AI 供应商跟随配置](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.1.7)

补丁:**①** 交叉透视「指标」多选(默认金额+占比双行)+ 宽度自适配铺满;**②** 预设看板按用户关心度重排,默认「资产类型」;**③** 打标页每行「改资料 →/持仓 →」直达维度数据源;**④** AI 洞察/打标跟随管理页主选 LLM 供应商(DeepSeek/通义)。无数据库变更。
...
| 测试 | JUnit 5 · 414 单元(含 PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 468 黑盒回归 |
...
bash scripts/qa-run.sh         # 黑盒 endpoint + 模板渲染(468)
