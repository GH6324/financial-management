...
### [v1.6.24 · 券商对接:在持仓页就能看到状态、一步去改配置](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.24)

用户提的路径很具体:**填报页 →「持仓管理」**。到了持仓页却看不出这个账户跟富途/老虎是什么关系,想改配置还得绕回账户页。现在持仓页标题下多一条**「券商对接」状态条**:已关联时一行给全 —— 券商 · 券商侧交易账户号(打码)· 同步启用状态 · 上次同步时间 · **本关联的 OpenD 地址**(没单独配就显示「全局默认 / 托管」),右侧【券商配置】【OpenD 网关】两个去处;没关联时**不摆空字段**(空态排一列「—」是假信息 —— 它让人以为"有这能力只是暂时没数据",真相是"根本没接",两者该给完全不同的下一步),只给【关联富途 / 老虎】。填报页账户行也加了「券商」直达入口。
**OpenD 向导现在会说明正在为哪个账户配置**,配完一键回那个账户的配置页 —— 向导本身仍是全局页,因为 OpenD 是**进程级**常驻服务、一台机器一个,按账户分没有意义,所以只补上下文提示、不改网关逻辑。
顺带说清模型:`broker_link` 上有 `UNIQUE(account_id)` —— **一个账房账户 ↔ 一个券商交易账户 1:1**,有多个富途/老虎账户就各建一个账户分别关联(整个账房对券商是 1:N)。这句话直接写在状态条下面,免得被当成"全局设置"。也修了一处判据不一致:账户页入口原先只对股票/加密/贵金属显示,而后端其实也接受基金/理财账户,现在两边统一。
