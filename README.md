...
现在每条疑似都带**第二视角**:`隐含损益 = 期末 − 期初 − 净流水`、`残留 = 隐含损益 + 疑似被抹的钱` —— 残留 ≈ 0 = 钱到期末**还没回来**(真要动手),残留远离 0 = 期末余额后来被改动过(**只提示核对**)。钱一补回,同一条痕迹**自动降级**,不需要手动标「已处理」。措辞与排序同步降级。**无 DB 迁移。**

**[v1.18.5](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.5) · 手填余额不再被估值抹掉 · 贵金属补进资产体检**
挂着持仓的账户(基金/股票/加密/贵金属)余额由**持仓合计**算出,于是你在填报页**直接敲的余额**会被下一次自动估值抹掉 —— 实测时间线:`14:42 手填` → `16:10 估值写回`,差额又没了。这是同一问题的**第三个变种**,被前两次修复从中间漏过去(v1.18.1 只管划转/收入、v1.18.3 的写回拦截只认**流水**,而手填两者都不是)。现在:你敲的 X 与(持仓+现金)的差额会**记成一行现金**,下次估值重算 = X,数字站得住,差额在持仓页看得见可改。
同版还修:资产体检的「投资类账户」判据漏了 v0.14 加的 **METAL** —— 贵金属账户被「持有期/收益/回撤」三条规则**静默跳过**。三条语义收成 `AccountType` 上的具名谓词,并加**结构性测试**:以后加新类型不表态就直接红。**无 DB 迁移。**
