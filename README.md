...
**[v1.22.1](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.22.1) · 手机端排版:核心信息不再被挤没**
填报页的流水行第一行是 4 列栅格,而有消费分类时这一行有**5 个**元素 —— 金额被挤到下一行,账户名压成「支付宝-余…」(**不是宽度问题,是列数不够**)。账户行更严重:账户名被硬编码 `max-width:120px` 锁死,而同行的标签全都不收缩,实测在 390px 下**被压到 2px**。现在账户名不参与压缩、金额锁在同行末尾、状态标签对齐成一条轴;全站 tag 收口(中文不需要 0.1em 字距)。宽图表补「可以横滑」提示 —— 它们本来就能滑,只是没线索。顺带修掉 **`−−¥499.00` 双负号**、两套负号约定并存(`¥-12,345` vs `−¥12,345`)、**0 元支出提交后凭空消失**。**无 DB 迁移 · 不改任何产品或计算逻辑。**

**[v1.22.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.22.0) · 导入确认页:从「我替你删了」改成「我给建议,你拍板」**
v1.21 把系统剔掉的行叫「**已剔除**」还给个「捞回来」—— 那个词描述的是**已经执行完的动作**,而做那个判断的依据是启发式的、**会错**;错了的后果是支出少算,你不会立刻发现。现在每行一个「**录入**」勾,系统只给**默认值 + 逐行理由**;没勾的叫「未录入」,什么都没被删。**0 元和负数能录了**(默认不勾),负数**原样记负数**。「已存在」从只读改成**可改分类与账户**(含批量),改账户会同步挪余额;已关账的期只能改分类。**含 DB 迁移 V60**(只放宽 `amount > 0` 约束,不改任何数据)。
