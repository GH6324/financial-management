...
**[v1.19.6](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.6) · 手机上问 AI 变成一下的事**
「超级 Agent」在手机上原来要点汉堡 → 展开 → 再点,三下才进得去;而横屏、目录、隐私眼都在右下角常驻,一下就到。AI 问答是三根支柱之一,入口深度不该排在一个显示开关后面。现在那一列浮钮里多了「超级 Agent」,位置在隐私眼**上面一格**(隐私眼保持最下不动 —— 那个位置离拇指最近,而且你已经用惯了)。手机上没沿用 PC 那个带文字的按钮:它更宽,窄屏会压住表格。**无 DB 迁移。**
