...
v1.21 把系统剔掉的行叫「**已剔除**」还给个「捞回来」—— 那个词描述的是**已经执行完的动作**,而做那个判断的依据是启发式的、**会错**;错了的后果是支出少算,你不会立刻发现。现在每行一个「**录入**」勾,系统只给**默认值 + 逐行理由**;没勾的叫「未录入」,什么都没被删。**0 元和负数能录了**(默认不勾),负数**原样记负数** —— 顺带修掉两处静默算错:一笔 −499 的退款冲正原本会被悄悄记成 +499 的**消费**,0 元支出会在提交后**凭空消失**。「已存在」从只读改成**可改分类与账户**(含批量),改账户会同步挪余额;已关账的期只能改分类。**含 DB 迁移 V60**(只放宽 `amount > 0` 约束,不改任何数据;默认行为与 v1.21.2 逐字一致)。

**[v1.21.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.21.2) · 批量导入不再干等 AI · 五个分桶都能点开核对**
传完账单页面停在「读到 428 笔」几十秒不动 —— 那是在同步等 AI 归类。现在**上传阶段根本不调 AI**,确认页秒开;AI 变成页面上一个按钮,只有它自己转圈,不点也能提交,**你手改过的行 AI 永不覆盖**。流水列表每页默认 **10 条**(可切 10/20/50/100)并新增**筛选器**(类目 / 分类 / 账户 / 成员,只列本月实际有的值)。顺带修掉报表「支出构成」逐笔抽屉从 v1.21.0 起一直为空的问题。**无 DB 迁移。**
