...
- **v1.17 设计文档(在研 · PRD 待评审)**:[`prd/v1.17.md`](prd/v1.17.md)(Docker 部署下的富途 OpenD 一键接入:今天 Docker 用户被要求自备镜像 + 借一台有桌面的机器 + 把券商密码写进 `.env`,而原生用户只是在向导页点几下。实测证据表明「容器里跑不了 OpenD」这个前提是错的 —— gtk3/fuse 是桌面版的依赖,命令行版在我们镜像基底上零缺失;顺带修掉过期的下载域名 / 文件名 / 启动参数,那几处今天在原生路径上也是坏的)
