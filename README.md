...
### [v1.6.3 · 旭日图小块聚合](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.3)

手机上旭日图有大量扇块既没名字也没数字 —— 这个问题改了三次才对。前两次都在"标注策略"上打补丁(调门槛、加引导线),真正的问题是**图型容量**:320px 的环里塞 59 个块、平均每块 6°,无论怎么标注都不可能可读。
改成数据可视化的通行做法 **Top N + Other**:同层过小的块合并成「其他 N 项」——「行业集中」内环 **59 块 → 6 块**、「平台安全」27 → 12,每块都有名字和占比;合并块点不下钻,会提示去看下方「切片排行」(完整列表在那里)。
顺带修掉两个隐蔽问题:响应式判断改用窗口宽度(原先用容器 `clientWidth`,该值依赖布局时机、同一元素能读到两个值,导致改参数不生效);图表渲染失败不再无声(原先错误被写进另一个组件的容器又被覆盖,页面上只剩一个转圈)。
...
| 测试 | JUnit 5 · 431 单元(含 AttributionEngine 归因两步法闭合 + RebalancePlan 核销规则 + PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 498 黑盒回归 |
