...
**[v1.11.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.11.2) · 报表页两个 UI/交互 bug**
**负债下降曲线的图例压住了图上的金额** —— 它是全项目唯一忘了关图例的单序列图,默认图例落在绘图区里;关掉后又露出第二层:12 个点每个都印 7 字金额标签,同一水平线叠成一串。按既有惯例改成只标首末点。**「钱花在哪了」切「按账户」整块消失** —— 不是报错,后端 200,是 `hx-select` 挑的 id 不在 HTMX 响应里,`outerHTML` 把整个 section 换成了空(趋势区 chips 同根因)。改成按 `HX-Target` 分流。
护栏教训:原来那条只查「属性写没写」,属性全在行为全坏 → 新护栏发**真实 HTMX 请求**断言目标 id 真的在响应里。另补回了 v1.11 缺失的 prd/tech-design(并加护栏钉住「在研版本必须有设计文档」)。**无 DB 迁移。**
