...
**[v1.18.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.2) · 账目对账:把「记了流水、钱却没进余额」的账找出来**
新增**只读**的 管理 → 账目对账。v1.18.1 修的是「今后不再丢」,这一页是把**已经丢的**找出来给你补 —— 也抓反方向(钱转走了、余额没扣掉)。判据看**时间线形状**:某次估值的 Δ 恰好等于它之前那段窗口里进出账户的钱的相反数(真实数据:`14:34:25 划出 −125,000` → `14:34:56 估值 Δ +125,000`),市场波动不可能精确到分。
判据被真数据推翻过两次(按期合计对不出来 —— 估值会把抹掉的动作如实记成事件,两边正好相消),过程写在页面和 `docs/qa-cases.md` 里。只看**当前由估值托管**的账户、只在**估值真跑过**的窗口上判;容差用管理页那个此前**存了却没人读**的 `unexplained_epsilon`。**无 DB 迁移。**

**[v1.18.1](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.1) · 修一个会丢钱的估值覆盖 · 建议尽快升**
往「挂着持仓的账户」(基金/股票/加密/贵金属)**划转或记收入**时,钱只加到当期余额、没记进该账户的现金 —— 下一次自动估值按「持仓合计」重算就把它覆盖掉,**那笔钱从余额里消失**,净资产跟着少算。反方向也修了:给**没有持仓**的股票账户记收入会凭空建一行现金,反而让它被估值接管、余额被压成只剩那笔(实测 11 万被压成 4200)。判据统一成「支持持仓 **且** 真的有持仓」。
同版还修:「谁赚得多/谁亏得多」把**转入**读成同额亏损(归因改锚最近已关账的月份并在页面写明);数据源接入页**API Key 存不进去**的死锁。**无 DB 迁移。**
