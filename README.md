...
右下角常驻入口(手机在导航里),问「我的钱都放在哪些平台」「有多少是自己在盯的、有多少交给产品了」。**AI 不做任何计算** —— 数字由系统按页面同一份口径算好交给它引用,每个数字都带账期、标着关没关账、点一下回到原页核对。看得见它在想什么(查了哪些维度、用了什么参数、查到了什么),跑完自动收起、你在读就不收。答案可以带图:饼/柱/折线/瀑布由系统按引用值绘制,更复杂的形状允许它直接写 HTML 跑在隔离沙箱里 —— **图上的点同样只能来自引用**。两条路可选:**本机直连**(不需要公网、不开任何对外端口)或**百炼托管 Agent**。另有一套只读 MCP / HTTP 接口可接你自己的 AI 客户端,默认关闭。顺带新增「托管形式」维度。**新增 7 张表(纯新增)· 默认关闭,不开时零感知。**
[v1.19.1](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.1) 补一个隐私洞:思考过程摘要里的金额没接上隐私开关(做截图时发现的);同时修掉反方向的过度模糊 —— 百分比不该被糊。

<table><tr>
<td width="50%"><img src="https://github.com/LuoDi-Nate/financial-management/releases/download/v1.19.0/pc_ask-conv.jpg"><br><sub><b>一次问答</b> · 数字带账期、点得回原页</sub></td>
<td width="50%"><img src="https://github.com/LuoDi-Nate/financial-management/releases/download/v1.19.0/pc_ask-think.jpg"><br><sub><b>看得见它在想什么</b> · 跑完自动收起,你在读就不收</sub></td>
</tr></table>
