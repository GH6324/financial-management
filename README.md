...
**[v1.9.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.9.0) · 知道自己落后了 —— 以及升级安不安全**
自部署工具有个结构性缺陷:装完之后没人告诉你有新版,你可能正踩着上个月就修掉的 bug。这一版**只做「查」**:版本徽记出一个 5px 圆点(不是红色角标、没有弹窗),管理页版本卡告诉你落后几个版本、这中间改了什么、**有没有 DB 迁移**(因为回滚只回 jar 不回 DB,这一格直接决定你能不能安全退回来)。迁移判定是 fail-closed 的:清单被截断就显示「无法确定」,绝不报「无 schema 变更」。
隐私摆在明面:每天一次 GET,不带版本号/实例标识/任何用户数据,介意就一键关掉、零后台请求。**不做无人值守的自动更新** —— 一次坏发布会同时打坏所有实例,而我们自己的发布流程恰恰有个人工闸门。
