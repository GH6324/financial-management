...
右下角常驻入口(手机在导航里),用人话问「我的钱都放在哪些平台」「有多少是自己在盯的、有多少交给产品了」。**回答里每个数字都带着账期、标着关没关账、点一下回到原页核对** —— 数字由系统算好交给 AI 引用,AI 不做任何计算,所以它没有机会把数字讲错。看得见思考过程(查了什么、查到了什么),跑完自动收起;你在读它就不收。答案可以带**图**:饼图/柱状/折线/瀑布由系统按引用值绘制,更复杂的形状允许智能体直接写 HTML,跑在隔离沙箱里。顺带新增「托管形式」维度(自己盯 / 交给产品 / 随时可取 / 不动)。
