...
### [v1.6.28 · 运维三件套:立刻备份 / 从备份恢复 / 一键诊断 + 常用操作一次说清](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.28)

一位用户指出:一键脚本跑完只给了一行「停 + 日志」,**启动、重启、重来、更新、恢复备份全都没说**,并让我以"刚拿到一个开源项目的开发者"视角自己审视一遍。结论:装之前和装的那一刻做得不错,**缺的全在"第 N 天"**。
新增 **`deploy/backup-now.sh`**(立刻备一份 —— 自动备份最多是"昨天那一份";备完**校验这份能不能解开**,不过就删掉不留假备份;给个目录参数可同时拷到宿主机,因为备份躺在命名卷里等于半个备份)、**`deploy/restore.sh`**(列出备份让你选;**灌入前先把当前库另存 `before-restore-*` 当退路**,退路本身也校验、不过就中止;停应用再灌、灌完起回来;手输 `RESTORE` 确认)、**`deploy/doctor.sh`**(一键收齐形态/版本/容器/磁盘/数据卷/备份清单/最近错误日志,**密码自动脱敏**,只读,可直接贴 issue)。一键脚本跑完还会打印**十一条常用操作**,README 与 `deploy/README.md` 也补了同一张 Docker 运维速查表。
顺带修正一处文档说谎:Docker 下备份实际是**每 24 小时**(容器启动起算)、文件名 `finance-*.sql.gz`,不是文档里写的"每周日 03:00"。**开发中的一个教训也记在这**:第一版备份脚本忘了压缩 —— 文件生成了、`du` 看得到,于是打印「✓ 已备份」,恢复时才发现不是 gzip;**报了成功的坏备份比没有备份更危险**,而且同一个错在"恢复前另存退路"那份上又犯了一次(那条"选错还能撤回来"的承诺是空的)。这只因为做了**闭环往返验证**(插标记→备份→删标记→恢复→标记回来、以及撤销一次恢复)才被发现 —— 只验"文件生成了"会放它上线。

### [v1.6.27 · 更新检查以「你真能拉到的镜像」为权威 + 查不到必须说出来](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.27)

一位用户在 v1.6.25 上 `git pull` 后重跑 `bash deploy/docker-up.sh`,输出「· 版本无变化:仍是 v1.6.25」,**后面一行都没有**。查下去是三件事,其中一件是发布流程的漏洞:**上一版的镜像根本没发出去** —— 构建时 Maven Central 瞬时 403 拉不到 Spring Boot 父 POM(配置没问题,上一版同配置构建成功),于是镜像仓库里没有那个版本、`:latest` 还是旧的,**用户怎么更新都拿不到**;而我们当时**没有检查构建结果**就宣布发布完成 —— 生产是直装部署确实更新了,但 **Docker 是主推安装方式,镜像没出就只发了一半**。现在发布流程有一道**必做闸门**:等构建结论,再直接探镜像仓库里那个 tag 是否**匿名可拉**,不在就中止;判据用"镜像能不能拉"而不是"构建有没有绿"(构建绿也可能没真推上去)——「生产健康 ≠ 发布完成」已写进流程文档。
另两件:**查不到最新版时脚本一声不响**(用户那边访问 `api.github.com` 不通,而实现是"查不到就返回",于是他无法区分「已是最新」和「查不了」)→ 现在明确说出来并给关闭开关;**对比对象换成"你真能拉到的镜像"** —— release 是"我们发布了什么",镜像才是"你能拉到什么",两者隔着一次构建、可能短暂不一致(构建中)也可能长期不一致(构建失败,正是本次)→ 改以镜像仓库的 tag 列表为权威,GitHub release 降为补充信息(`✓ 已是最新可用镜像(vX)` +「GitHub 上已发布 vY,但镜像还没推上来 —— CI 约 12 分钟;久等不来说明构建失败了」)。顺带:国内直连镜像仓库比访问 `api.github.com` 稳得多。
...
| 测试 | JUnit 5 · 431 单元(含 AttributionEngine 归因两步法闭合 + RebalancePlan 核销规则 + PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 517 黑盒回归 |
