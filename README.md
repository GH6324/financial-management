...
**[v1.11.1](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.11.1) · 5 条使用反馈修复 + 测试基建体检**
**发版后不用再重新登录**(session 在进程内存里,重启即失效;remember-me 早就是 JDBC 持久化的,只是复选框默认没勾)。报表页资金流瀑布的**数字不再被遮住** —— 主因是截断轴的斜纹带横贯整宽、正好从溢出容器的 X 轴标签中间穿过。目标进度百分比保留 **1 位小数**(原来 8.4% 显示成 8%、0.4% 成 0%,月度推进全被吃掉)。长文目录**统一不编号**改用缩进表达层级。一页只保留**一个时间筛选器**(支出构成的窗口改为跟随趋势区)。
顺带做了轮测试基建体检:全量黑盒回归 41 条红**逐条查完一个真缺陷都没有** —— 一半是 qa-run 自己往真库写却不还原(把当期关账了导致几十条级联假红),一半是护栏钉在已被推翻的旧规则 / 界面文案 / 写死的计数分母上。现在 **577/0 黑盒 · 93/0 端到端 · 506 单测**。**无 DB 迁移。**

**[v1.11.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.11.0) · 13 条反馈一次做完(性能 −55% / 局部刷新 / 指标审计)**
**性能**:dashboard 1.735s→0.784s(−55%)、报表 1.247s→0.784s(−37%)—— 把「每期首次出现账户」的 per-period 查询合并成一次窗口函数扫描。**交互**:趋势区与支出构成的筛选器从整页跳转改 HTMX 局部替换(不丢滚动位置);长文目录最后一项现在能高亮了。**新增**:报表页二区加「按成员 / 按资产大类」两块封板分布。
**指标审计**([`docs/metric-audit-v1.11.md`](docs/metric-audit-v1.11.md)):47 项指标逐项对齐口径,发现并修掉「缺当年 CPI/M2 时静默用历史均值」与「收支稀疏导致比率显示 −2383%」两条。另做了一轮**信息安全脱敏**(审计/复盘文档与手册页里的真实金额,80+ 处)+ 落护栏。**无 DB 迁移。**
