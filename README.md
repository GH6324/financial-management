...
### [v1.6.26 · 数据安全:清理链全部改「宁可不删」+ 删前强制备份 + 全新空库主动告知](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.26)

一位用户报:装好后设了成员、改了密码,更新完发现**多了两个叫 Alice / Bob 的成员,自己的账户被清空了**。**先定性**:Alice / Bob 是内置两个账号(`diwa`/`wangergou`)的默认显示名,来自建库种子;而建表脚本用的是**裸 `CREATE TABLE`**(没有 `IF NOT EXISTS`)—— 迁移一旦在已有库上重放,第一步就会失败并中止,所以"迁移把种子重灌进你的库"物理上不可能。剩下**只可能是连上了一个全新的空数据库**:数据卷被换或被删(目录名变 → compose 项目名变 → 新卷,或执行过 `down -v`)。**责任在我们** —— 上一版处理"数据库密码不匹配"时把 `down -v` 作为并列的第二条出路,还写着"那个数据库你从没真正用过就可以整卷删掉";刚建过成员改过密码的用户凭记忆判断"我没什么数据"极易判错,而删卷不可恢复。
现在:**全新空库会主动告诉你**(并给 `docker volume ls | grep db-data` 自查 + 两条回到旧数据的路);**`down -v` 降为第三选项**,前两条都不丢数据(放回旧密码 / 保住旧卷换新项目名);清理演示数据的**互锁改成"查不出来就不删"**(原写法 `$(查询 || echo 0)` 查询一失败当 0 = "可以清",**保命互锁在自己失效时选了破坏性那边**);判断"用户用过没有"补上**只有真人交互才会改变的字段**(是否完成首登改密、种子成员是否改过名);**删之前强制备份**,备份失败就不清 —— 价值不在"判断更准",而在**把不可逆变成可逆**。另外"没有迁移记录"不再等于"库是空的":恢复过备份 / 手工建库 / 迁移中断都会这样,业务表有行就一律按非全新库处理。

### [v1.6.25 · 更新路径会说话:`/health` 带版本 + 脚本报「从哪一版升到哪一版」](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.25)

一位用户报:`git pull` 后重跑 `bash deploy/docker-up.sh`,跑完还是旧版本。端到端复现后结论有点意外 —— **脚本的更新能力本身是好的**(镜像退回旧版、起旧容器、跑脚本,容器确实换到了新镜像)。真正的问题是三件事叠加:**`git pull` 拉到的新代码不进容器**(应用来自预构建镜像,`git pull` 只影响 compose 文件与部署脚本本身)、**打 tag 到镜像可用之间有约 12 分钟 CI 构建**、以及**脚本从头到尾不说版本**(`/health` 只有 `status`,落地页没有,徽记只在登录后的导航栏里)。第三条是困惑的直接原因:脚本无论有没有更新成功,输出都是同一句「✓ 起好了」。
现在 `curl -s http://127.0.0.1:20000/health` 直接给 `{"status":"UP","version":"1.6.25"}`;脚本会报 `✓ 已更新:v1.6.24 → v1.6.25`,落后时还会分清"你该等一会儿(镜像还在构建)"和"你已经最新";README 也有了独立的「[更新到新版本](#更新到新版本)」小节。**没有新增 `update.sh`** —— 同一条命令既是首装也是更新,多一个入口就多一个"我该跑哪个"的问题。
顺手修掉两个既存缺陷:**未登录遇到错误时错误页自己会挂** —— 真因是 Thymeleaf 属性优先级(片段包含比条件判断先执行,于是"仅登录才用完整导航栏"的条件没生效,导航栏拿着空数据渲染抛异常、**响应截断在导航栏中间**,还会**盖住真正的报错**),已修并把同类写法全仓清零;错误页头部改为完全自包含 —— **兜底页面不该依赖刚刚出错的那套机制**。
...
| 测试 | JUnit 5 · 431 单元(含 AttributionEngine 归因两步法闭合 + RebalancePlan 核销规则 + PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 514 黑盒回归 |
