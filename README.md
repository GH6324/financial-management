...
- **v1.12 设计文档(在研)**:[`prd/v1.12.md`](prd/v1.12.md)(把「封板」做实:关账时把账户的分类属性一并定格,改设置不再改写历史月份的集中度/流动性分层/大类分布/vs 基准 —— 只冻结**分类输入**、不冻结**指标输出**,保住「口径改了自动对全部历史生效」;顺带补 SQL 归因手段并消掉报表页剩余 N+1)
