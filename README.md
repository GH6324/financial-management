...
- **支出分类(可选)** — 想知道「钱花在哪」再开:自建类目树,**一棵树两种深度**(简单版只用大类 / 复杂版多长一层,顶层完全相同,中途切换不丢钱)· 填的是**每个类目一个月一个数**,合计自动写回月度总额 —— 是总额的展开,**不是逐笔流水账** · 支付宝/微信月账单 csv 一键导入(**金额永不进 LLM**,文件不落盘)· 一格钱按来源分行存,**重导不会冲掉你手工改过的数** · 不开就一个像素都不多
...
- **v1.21 设计文档**:[`prd/v1.21.md`](prd/v1.21.md) · [`tech-design/v1.21.md`](tech-design/v1.21.md) · [`preview/v1.21/expense-categories.html`](preview/v1.21/expense-categories.html)(源自 [issue #18](https://github.com/LuoDi-Nate/financial-management/issues/18) —— **自定义支出分类**,和「刻意不做逐笔流水账」这条老边界正面冲突,所以改的是边界的**位置**而不是取消它:分类做到「**月度按类目汇总**」为止,是月度总额的**展开**(合计自动写回总额),**不是第三种记账方式**;再往下的逐笔流水仍然刻意不做。两个设计决定值得单说:① **一棵树、两种深度** —— 简单版/复杂版**顶层完全相同**,复杂版只是在同一棵树上多长一层,所以中途切换不需要任何映射表,「映射」就是那条 `parent_id` 边;从复杂切回简单,钱**留在原地**只是显示上收到父级,切回来一分不少。② **来源行模型** —— 一格钱按来源分行存(手工 / 支付宝 / 微信 / 截图),重导只删**那一条通道**的行,所以**用户的手工订正永远不会被下一次导入冲掉**;想改哪个数就直接改,改的是「手工那一份」。账单导入走渠道自己的月账单 csv(不做银行 OCR、不接支付平台 API),**金额永不进 LLM**,文件不落盘、不进日志。)
