...
往「挂着持仓的账户」(基金/股票/加密/贵金属)**划转或记收入**时,钱只加到当期余额、没记进该账户的现金 —— 下一次自动估值按「持仓合计」重算就把它覆盖掉,**那笔钱从余额里消失**,净资产跟着少算,每跑一次估值再抹一次。反方向也修了:给**没有持仓**的股票账户记收入会凭空建一行现金,反而让它被估值接管、余额被压成只剩那笔(实测 11 万被压成 4200)。判据统一成「支持持仓 **且** 真的有持仓」,记账与估值共用一份定义。**已经丢的钱不会自动补** —— 升级后去那个账户的持仓页加一行现金补上即可。
同版还修:「谁赚得多/谁亏得多」把**转入**读成同额亏损(归因原来看当月,而当月「转账已登记、余额没填」;现在锚最近已关账的月份并在页面写明);数据源接入页**API Key 存不进去**的死锁(密钥与模型选取拆成各自保存)。**无 DB 迁移。**

**[v1.18.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.0) · 流水时间线现在说清「这笔是谁写进来的」· 券商同步失败会标在账户上**
流水时间线以前只说得清「是收入还是估值」,说不清数据从哪来。现在每行带来源标签(手动填报 / 自动 · 股价·金价·币价 / 自动 · 富途·老虎 / 截图导入 / 开账延续 / 系统联动),按来源分组上色。历史数据一律标「来源未记录」,不假装成手动填报。同版把券商同步失败标到**账户列表**上。**含 DB 迁移 V56:4 张表纯 `ADD COLUMN`,不动金额,回滚安全。**
