...
配好主选模型、**取消勾选**「启用持仓截图导入」,保存却报 `截图识别:请选择平台` —— 一个已经关掉的功能凭什么还要求你填。根因是主选/备选/截图三组走同一套校验,而它一律要求平台选得出来;只配了 DeepSeek(无视觉能力)的人更是死路:视觉下拉一个可选项都没有,关掉它还是存不下去。现在按「你想干什么」分支:关掉 → 那一组一个字不校验(旧配置原样保留,填错也只提示不拦人);想开但没有有视觉能力的平台 → 勾选框**直接禁用**并写明去配哪家;选了没配密钥的平台 → 当面拒绝(以前能存,下次调用才失败)。
火山方舟以前只给一句「这一家没有可预置的型号」,连去哪个页面复制都不说 —— 重新调研后那个前提已不成立(方舟现在支持**直接填 Model ID**,不必建接入点):型号下拉已预置推荐型号,默认取**不带日期**的 `doubao-seed-evolving`,并给出开通管理/模型广场直达链接;手填照旧可用。**无 DB 迁移。**

**[v1.18.3](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.3) · 归因回到「当月实时」· 自动估值不再可能盖掉刚进的钱**
仪表盘的归因复盘挪回**当月实时**(上月关账时点 → 现在)。v1.18.1 曾把它锚到上一个已关账月来绕开假亏损,但那个假亏损的**病根已经修掉**,而锚在上月会让同一屏出现两个月份、很容易看成算错。仪表盘=当月实时、报表页=已关账封板,分工不变;代价照实写在页面上。
同版补上最后一块:**估值写回前先拦一道** —— 余额那张快照是覆盖写、被盖掉的值没有任何留底,是系统里唯一不可恢复的自动写。若某次写回恰好会抹平刚进出账户的钱,就**拒绝覆盖**并记进「管理 → 账目对账」。**无 DB 迁移。**
