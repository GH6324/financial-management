...
**[v1.19.3](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.3) · 支出可以记在信用卡上**
线上反馈「支出页选不中信用卡」。原设计把「贷款」类整个排除在支出账户之外,理由是「在贷款账户上记支出等于又借了一笔,不是花钱」—— 这话对房贷成立,但**刷卡消费恰恰同时是借钱和花钱**。现在信用卡可选,记一笔消费会让这张卡欠得更多。同时挡住一类很难自查的错账:卡上记「消费」+ 还款记「还贷」= 同一笔钱算进本月支出两次(报表上每个数字都是真的,只是被算了两次),所以负债账户上不再提供「还贷 / 利息支出」,那两笔要记在钱实际流出的现金账户上。
已知边界:**没有**新增「信用卡」账户类型,所以房贷/车贷也会出现在支出下拉里,需要自己不选。**无 DB 迁移**。

**[v1.19.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.0) · 超级 Agent —— 用大白话问你自己的账**
右下角常驻入口,问「我的钱都放在哪些平台」「有多少是自己在盯的」。**AI 不做任何计算** —— 数字由系统按页面同一份口径算好交给它引用,每个都带账期、点得回原页核对;看得见它在想什么,跑完自动收起、你在读就不收;答案可以带图(饼/柱/折线/瀑布,复杂形状让它写 HTML 跑在沙箱里),**图上的点同样只能来自引用**。两条路:本机直连(不需要公网)或百炼托管 Agent;另有只读 MCP / HTTP 接口可接你自己的 AI 客户端。
新增 7 张表(纯新增)· **默认关闭**,不开时零感知。
[v1.19.1](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.1) 补隐私洞(思考过程摘要里的金额没接上隐私开关)· [v1.19.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.19.2) 「AI 接入」页补百炼接入教程(业务空间 ID / MCP 服务 ID 怎么拿、权限与地域要求),并修掉一段抄过去用不了的配置 JSON。
