...
流水时间线以前只说得清「是收入还是估值」,说不清数据从哪来 —— 一笔估值变动,分不出是定时拉股价、富途同步、还是自己截图导入的;「月末校准」那行是你敲的数还是开账时系统代填的,也长得一模一样。现在每行带来源标签(手动填报 / 自动 · 股价·金价·币价 / 自动 · 富途·老虎 / 截图导入 / 开账延续 / 系统联动),按来源分组上色。
**历史数据一律标「来源未记录」,不假装成手动填报** —— 库里没有依据可推断,标成手动会让统计得出「过去全是手填」的错误结论。同一版把券商同步失败标到**账户列表**上(以前只在二级页可见,而那页没人天天点)。**含 DB 迁移 V56:4 张表纯 `ADD COLUMN`,不动金额,回滚安全。**

**[v1.17.3](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.17.3) · 修富途同步静默中断(10.8 被误判)· 失败不再假装成功**
**用富途同步、且 OpenD 是 10.9 或更早版本的,请升这一版。** v1.17 把启动方式按「主版本 ≥ 10」分流,而我只实测过 10.10 —— 装 10.8 的机器上 OpenD 起来就退出,同步静默中断。边界改钉在实测过的 10.10,更早的走回老参数;规则写进护栏:**没实测过的版本一律走老路径**。**无 DB 迁移。**
