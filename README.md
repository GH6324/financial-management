...
### [v1.6.17 · 控制按钮不再等加载 · 维度顺序重排 · 持仓方向横条](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.17)

**右下三个控制按钮**(横屏 / 本页目录 / 隐藏金额)原来要等旭日图加载完才浮现 —— 不是设计,是 bug:首屏印章过渡层原本挂在 `window.load` 上,而 `load` 要等**所有子资源**(图表库、脚本、图片),重图表页面能拖两三秒,按钮被压在过渡层下。系统级能力不该被页面内容的加载进度绑住 → 过渡层改挂「结构就绪」+ 按钮层级抬到它之上,资产透视页从「等全部加载完」变成 **0.7 秒**可点。
**「下一层按」维度顺序重排**:原来第一个是「风险」。依据是下钻的动机是"再切一刀看结构",最常问的是结构性问题;风险 / 流动性是属性判断且各有专门看板 → **结构类**(资产类型→平台→行业→主理人→用途)+ **属性类**(风险→流动性→币种→地域→账户类型)。同时「成员结构」看板第二层从风险改成资产类型。
**方向图标**补上旋转弧(原来只有"横屏手机",传达不出"旋转");小尺寸那两处不加 —— 弧会糊成一团。**子页面控制按钮**不再缺位:三个按钮原来各写死距底距离,某页少一个就留空洞,改成纵向容器"有几个排几个"。**打标页「持仓方向」**加回最初设计稿的横条占比示意,标签改横滑不再换行。

### [v1.6.16 · 饼图标签上环 · 旭日零撞色 · 横屏点得动](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.16)

**饼图**:名称和占比直接标到环上(之前环上只有百分比,名称金额都在图例,要来回对照)。阈值按**弧长**算而不是按百分比拍死 —— 弧长 = 2π × 中半径 × 占比,才是"这块放不放得下字"的真实约束。
**旭日下钻不再撞色**:查下来每环色板只有 **10 色**,而外环(行业/平台/账户)动辄十几二十个值,超出直接复用。扩容用「**旋转色相、保持明度**」—— 一开始试往纸色/墨色混,不同底色混到一半会**收敛成几乎同色**(等于还是撞色);旋转色相则明度不变,内环始终在暗半区、外环始终在亮半区(默认方案实测:内环亮度 0.08–0.36 / 外环 0.38–0.72,两带不相交)。容量 10 → **内环 77 / 外环 45**,11 个看板逐个核对:真实维值 **0 撞色**、内外环 **0 共用色**。
**横屏**:退出按钮原来只有 26px 高,横过来在屏幕上是 26px 宽的一条,远低于 iOS 建议的 44pt 触摸目标 → 按钮 34px、整行 44px,并去掉双击缩放的点击延迟。横竖屏切换后图表不再错位:图表库的自适应在容器刚变化时读到旧尺寸、另一个库根本不跟容器,改为显式按容器尺寸重排 —— 实测 9 个图(含归因瀑布、12 月趋势、旭日)三态下全部精确贴合。
**打标页手机端**:先量才知道瓶颈是 62 行 × 卡片化约 13800px(说明段只占 186px)→ 加「只看未打标」过滤(手机默认开,只隐藏不影响提交)+ 底部常驻保存条。
...
| 测试 | JUnit 5 · 431 单元(含 AttributionEngine 归因两步法闭合 + RebalancePlan 核销规则 + PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 505 黑盒回归 |
