...
### [v1.6.20 · 打标页手机端强制横屏,布局回到电脑版表格](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.20)

「给资产打标」是全项目最像**表格作业面**的一页(6 列 × 62 行)。竖屏只能把每行拆成一张卡片 —— 一行占一屏、整页一万三千多像素,几乎没法干活。现在手机进这页**自动切横屏**,布局回到**电脑版表格形态**(表头 + 六列并排):第一行数据距顶 **278px**(压缩页头前是 418px),整页高度 **~13800px → ~2400px**。
三个细节:**只自动切一次** —— 手动点「退出横屏」后本次访问不再自动切,不跟用户抢方向;**电脑端完全不受影响**;横屏下**说明段仍保持折叠**(横屏竖向空间 390px 比竖屏更紧,说明全文正是最该收的),「只看未打标」筛选和底部「保存全部」都留着。
**顺带一条工程结论**:横屏模式的原理是"锁布局宽度为屏幕长边 + 旋转 + 原地切换响应式断点",而"切断点"只作用于框架自带断点 —— **页面自己写的窄屏样式不受影响**(视口仍是短边)。所以每页接入横屏都要显式排除自己的窄屏规则,否则横屏会被按回手机形态;这条已写进技术文档。
