...
仪表盘的归因复盘挪回**当月实时**(上月关账时点 → 现在)。v1.18.1 曾把它锚到上一个已关账月来绕开假亏损,但那个假亏损的**病根已经修掉**(钱没记进现金、被估值抹掉才被读成亏损),而锚在上月会让同一屏出现两个月份、很容易看成算错。仪表盘=当月实时、报表页=已关账封板,分工不变;代价照实写在页面上(「还在填报中 · 本月已录收入/支出 · 收支录得越少「钱赚」越偏高」)。
同版补上最后一块:**估值写回前先拦一道** —— 余额那张快照是覆盖写、被盖掉的值没有任何留底,是系统里唯一不可恢复的自动写。若某次写回恰好会抹平刚进出账户的钱,就**拒绝覆盖**并记进「管理 → 账目对账」的「被拦下的写回」。判据与对账扫描**共用一份**。**无 DB 迁移。**

**[v1.18.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.18.2) · 账目对账:把「记了流水、钱却没进余额」的账找出来**
新增**只读**的 管理 → 账目对账。v1.18.1 修的是「今后不再丢」,这一页是把**已经丢的**找出来给你补 —— 也抓反方向(钱转走了、余额没扣掉)。判据看**时间线形状**:某次估值的 Δ 恰好等于它之前那段窗口里进出账户的钱的相反数(真实数据:`14:34:25 划出 −125,000` → `14:34:56 估值 Δ +125,000`),市场波动不可能精确到分。只看**当前由估值托管**的账户、只在**估值真跑过**的窗口上判。**无 DB 迁移。**
