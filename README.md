...
### [v1.6.15 · 横屏切换从 1.4 秒降到 0.3 秒](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.15)

点「整页横屏」原本要卡一下 —— 实测 **1362ms**。原因是那时横屏靠一个同源 iframe 拿"独立视口",而 iframe 是一份新文档:等于**重新渲染一整个页面**(一次后端请求 + 12 个脚本重跑);副作用是新文档没有旧的滚动位置,从「旭日下钻」这类靠下的章节切过去会回到顶部。
这个项目的 Tailwind 是**运行时编译**的版本,重新给一份配置它会就地重编译(实测 114ms)—— 所以 iframe 完全不必要。改成原地换断点后:切换 **1362ms → 约 300ms**、页面请求 **1 → 0**、脚本重跑 **12 → 0**,而且切过去/切回来都停在原来那个章节。
顺手修三处:退出横屏按钮不再贴屏幕边缘(iPhone 圆角会切到贴边元素、按不稳),改与「本页目录」「隐藏金额」同排在导航行;方向图标换成图标库标准图标(Tabler),按钮显示**目标状态**;旭日下钻配色改为**跟着配色方案走** —— 之前风险维度是一条写死的色阶,与莫兰迪等方案不同族且换方案不变,现在五套方案各带一条有序色阶与一组中性色。
