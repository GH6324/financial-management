...
### [v1.1.1 · 透视性能根治 + 打标体验重做 + AI 推荐修准](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.1.1)

补丁:**① 性能** — 透视缓存重设计(过期不阻塞 · 启动预热 · 写路径事件全覆盖),根治 v1.1.0 每分钟踩一次同步冷组装的周期性卡顿,查询稳定 10–60ms;**② 体验** — 打标页双端重做(PC 固定列宽 · 手机卡片化 · AI 预填改控件高亮),旭日/排行**同一维值全局同色**;**③ AI 推荐修准** — prompt 按「底层投向」语义重写+few-shot(余额宝不再误标金融地产);**④** 「大类资产」更名「资产类型」· 打标入口增至三处。无数据库变更。
...
| 测试 | JUnit 5 · 414 单元(含 PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 465 黑盒回归 |
...
bash scripts/qa-run.sh         # 黑盒 endpoint + 模板渲染(465)
