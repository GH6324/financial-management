...
**[v1.22.2](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.22.2) · 手机端:账户卡与流水分页**
填报页的账户卡有四处不对:那个绿勾**没人看得懂**(它表示「本期已填」,但视觉语言是「可点击的复选框」,而它点不动也没说明)、账户名被挤到第二行、刷新按钮单独占一行、**「上期末」在一张卡里出现两次**(确实渲染了两次)。逐个修掉,三个入口靠右对齐。流水分页:**手机端每页 5 条**(桌面仍 10),列表**底部多一组翻页**。有意没做吸顶(工具条两行,吸顶吃掉近三分之一可视高度)和无限滚动(这是**前端分页**,数据早已全量在页面里,无限滚动省不掉加载只会丢掉「第几页/共几页」,而核对账目恰恰需要位置感)。**无 DB 迁移 · 不改任何产品或计算逻辑。**

**[v1.22.1](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.22.1) · 手机端排版:核心信息不再被挤没**
填报页的流水行第一行是 4 列栅格,而有消费分类时这一行有**5 个**元素 —— 金额被挤到下一行,账户名压成「支付宝-余…」(**不是宽度问题,是列数不够**)。账户行更严重:账户名被硬编码 `max-width:120px` 锁死,实测在 390px 下**被压到 2px**。现在账户名不参与压缩、金额锁在同行末尾;全站 tag 收口。宽图表补「可以横滑」提示。顺带修掉 **`−−¥499.00` 双负号**、两套负号约定并存(`¥-12,345` vs `−¥12,345`)、**0 元支出提交后凭空消失**。**无 DB 迁移。**
