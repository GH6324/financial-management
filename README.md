...
**[v1.10.0](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.10.0) · 报表页从「收益分析页」变成「月度封板快照」**
两页分工定下来:**仪表盘=当期实时**(会变)· **报表页=关账后的封板快照**(不再变)。报表页重构成三区,时间范围只作用于「趋势」区 —— 六个范围的前两区渲染**指纹逐字相同**。新增期末资产负债表、资金流瀑布(同时是对账表)、环比/同比三列对照、集中度(带上期幽灵条)、流动性分层、本期归因。
顺带修掉一个 bug 级问题:**归档一个账户会改写全部历史快照**(裸 `archived_at IS NULL` 让归档抹掉该账户的全部历史事实)。仪表盘「本月资产收益」改成实时本月 + 讲清偏差方向。**无 DB 迁移。**
