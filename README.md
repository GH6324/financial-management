...
### [v1.6.18 · 隐藏金额两处漏网修复 · 环上补金额 · 横屏跨页面](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.18)

**隐藏金额两处漏网(重点)**:①「本期怎么变的」里的「本期收入 − 支出」压根没被标记为需要遮罩,开了隐藏也是明文;②「归因」那块的数字画在**画布**上,而隐藏金额是 CSS 模糊、管不到画布像素 → 现在两件事都做:生成文案时就判隐私 + **切换开关后重新出图**(已画上去的像素不会自己变)。顺带网住这一类:仪表盘模块里**所有**绝对金额必须带遮罩标记,以后新加的会被回归测试拦下。
**饼图**环上补**名称 / 金额 / 占比**三行(三行需要 33px、环带有 42px 放得下;能否放下仍按弧长判断,隐私态自动降两行)。**横屏跨页面保持**:之前点账户详情跳过去就变回竖屏;关键不在"记住"而在**时机** —— 必须在样式首次计算前落地,否则新页面先按竖屏排一遍再被扳过去。
**交叉透视指标放行**:下钻能力一直是好的(两种摆法都会打开明细),问题是**看不见它开了** —— 明细在表格下方,指标放行时表格高数倍,而原滚动策略只滚"最小距离" → 改为直接带到视口顶部 + 闪边框确认。**隐藏金额时浮动按钮不再偏移**(隐私按钮变宽把整列对齐基准撑歪了 → 改右对齐)。
...
| 测试 | JUnit 5 · 431 单元(含 AttributionEngine 归因两步法闭合 + RebalancePlan 核销规则 + PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护)/ 55 e2e 断言(11 主线)/ 507 黑盒回归 |
