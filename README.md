...
### [v1.6.30 · 全站指标核查:收益率不再把「还没录的收支」算成投资收益](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.30)

这版不是修某个报错,是一次**主动体检**:把所有页面的指标列成清单,逐个核 ① 算得对不对 ② 计算逻辑对不对 ③ 指标定义本身对不对(完整报告 [`docs/metric-audit-2026-08-01.md`](docs/metric-audit-2026-08-01.md))。**先说结论:计算正确性本身没查出错** —— 24 个指标都能从原始数据表精确复现,三条恒等式全部成立。问题出在**锚点选错**:取数是「账户 × 账期」全交叉且**不筛账期状态**,于是**还在填报中(未关账)的当月被当成"最后一期"**,而它的典型状态是「余额已填了、收支还没录」→ 收益率公式里的净流入是 0 → **你这月还没录的工资,被整块算成了投资收益**。我们生产库实测:当月 21 个账户余额全填、收支 0 条,「本月资产收益」显示 +0.99%,那 9.1 万变化一分钱没扣。新规则:**存量类**(净资产/总资产/总负债/流动资产/环比)**继续看最新一期**(填报中本来就该看到最新余额),**收益类**(本月资产收益/XIRR/TWR/本年累计损益/人赚钱赚/储蓄率)**锚最后一个已关账账期**,并且页面**明说算的是哪个月**(标题变「资产收益 2026-07」+ 说明写清当前月仍在填报故不参与计算)。
顺带修掉四个:**①「储蓄率达到 X%」的目标进度一直显示 0%**(它读的那个实现只认逐账户流水、又锚在填报中的当月 → 算不出来 → 兜底成 0,而报表页同名指标显示 96.66%);**②「总资产」的口径说明漏了加密/贵金属/保险三类**(数字一直对,错的是那句说明);**③ 同一个「家庭 XIRR」体检页 −0.21%、报表页 +1.11%**,还分别走了"累计"和"年化"两条分支(各页取数窗口宽度不一致,一边凑够 12 个已关账月、另一边只有 11 个)—— 现在统一取「最近 12 个已关账月」;**④ 体检页「本年累计损益」的负号被吃掉,亏损显示成盈利**(亏 158 万渲染成「¥1,580,715」,只有字的颜色透出亏损)。第 ④ 条是**双端逐屏看渲染结果**才发现的:语法合法、指标值也算对了,错在展示,自动化检查抓不到。
⚠️ **升级后 5 个数字会变**(我们生产上:本月资产收益 +0.99%→+4.27%、XIRR +0.79%→**−0.20%**、TWR +0.70%→−0.28%、储蓄率目标进度 0%→96.66%,净资产不变)。**−0.20% 不好看但它是真的** —— 三个已关账月投入 924.999 万、期末 923.155 万,确实略亏,之前的 +0.79% 是虚高。无数据库迁移、无存量数据改写、纯读取路径,回滚只回退程序。**静默改动用户看到的数字是信任成本最高的一种改动**,所以逐项列出来。

### [v1.6.29 · 报表 XIRR:说明文字与指标同源 + 收入口径全页统一](https://github.com/LuoDi-Nate/financial-management/releases/tag/v1.6.29)

用户报「生产上新关一个账期后,报表的家庭 XIRR 显示 0.00%,不对吧」。**先复算再改**:在真实数据上还原 —— 6 月投资损益 −33.03 万、7 月 +33.03 万,合计 −1 元,窗口内正负相消,**XIRR 本身算得是对的**。让人觉得不对的是**旁边那段说明文字**:它写「期初净资产 **−¥0** → 期末净资产 +¥733,258」,而这两个数**都不是净资产** —— 取自另一条序列(剔除累计开账基线的趋势,那是给财富水位图用的),那条序列**第一个点按构造永远是 0**(首期所有账户都算"首次纳入"),真实期末净资产是 923 万。**一个号称"给你看真实中间数值"的说明显示了另一套数,比没有说明更糟** —— 用户会据此判定指标算错并来找我们查。现在它与指标同源,并把「中途纳入的存量账户本金」单列说清;顺带修了同一处第二个错:原文案不管几期一律写"求解**年化**",而不满 12 期用的是累计口径。
第二个缺陷:**同一屏两个指标读两套账** —— 「家庭 XIRR」只读逐账户流水,「人赚 · 净流入」优先用你在填报页填的家庭收支,于是同屏互相矛盾(7 月填的 21,837 支出「人赚」扣了、XIRR 没扣)。现在 XIRR / TWR / 本金vs收益分解三者共用同一口径,**结构上不可能再漂**。
⚠️ **升级后这个数字会变**(我们生产上 `0.00%` → `-0.20%`)—— 是**修正不是变差**,原值漏扣了已填报的支出。无 schema 迁移、无存量数据改写,回滚只回退程序。**静默改动用户看到的数字是信任成本最高的一种改动**,所以专门说明。
...
| 测试 | JUnit 5 · 438 单元(含 AttributionEngine 归因两步法闭合 + RebalancePlan 核销规则 + PivotEngine 透视引擎(归因降级+币种不变性) + LensAiTag 白名单 +  PrivacyIsolationTest 静态扫源码私密红线 + CurrencyInvarianceTest 币种不变性(含保险) + AShareTicker 交易所前缀 + MetalUnit 贵金属单位/归一 + BrokerReadOnlyGuard 券商只读铁律静态扫 + FutuOpend 向导只读护栏(下载白名单/只绑127.0.0.1/密码只MD5)+ AllocationDiff 保险独立桶 + InsurancePolicy 保单登记 + EntryLoanPrompt 贷款趋势预测兼容闸 + GoalMetricEvaluator 指标聚合 + GoalPaceCalculator 进度落后判定 + 单一镜头端到端币种守护 + ClosedPeriodAnchorTest 收益类指标锚已关账期)/ 55 e2e 断言(11 主线)/ 518 黑盒回归 |
