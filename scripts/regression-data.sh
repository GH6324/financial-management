#!/usr/bin/env bash
# =============================================================================
# regression-data.sh · 数据层 / 口径回归(原 scripts/e2e.sh)
#
# ── 2026-09-20 正名:这个脚本不是 e2e ──
#
# 它用 curl 打端点 + mysql 查真值,验的是「**接口通了、库里算对了**」。
# 那是**数据层回归**,很有价值,但它跳过了用户与系统之间的全部东西:
# 表单字段名、按钮可达性、JS 绑定、渲染完整性、并列元素尺寸。
# 而这个项目的事故恰恰大量出在那一段 ——
#   · 分桶页签绑错作用域,点了没反应,curl 200 一切正常(v1.21.2)
#   · chunked 渲染中途炸掉,curl 照报 200,浏览器里是半张白页(v1.19)
#   · 账户名被 max-w 压成 2px(v1.22.1)
#   · 填报页的账户下拉是自研件,curl 直接 POST accountId 永远成功,
#     而用户点的是另一套 DOM(v1.23 的浏览器 e2e 第一次跑就抓到了)
#
# **真 e2e 在 scripts/e2e/**(Playwright 驱动浏览器,动作一律从页面元素发起)。
#
# ── 那为什么不把这个脚本删掉 ──
#
# 因为它里面 119 处 SQL 断言验的是**计算口径**:XIRR 币种无关、精确算术
# (期末 = 起点 + 收入 − 支出 − 转出)、归因锚点、开账基线、NAV 不被估值抹掉。
# 这些**没有 UI 路径** —— 用户界面上根本没有「XIRR 的币种不变性」这个按钮。
# 硬塞进浏览器不会让它更真实,只会让它更慢更脆。
#
# 所以两者职责分开,都要跑:
#   scripts/e2e/run.cjs        真 e2e   · 用户点得到吗、页面对吗、库里也对吗
#   scripts/regression-data.sh 数据回归 · 口径算对了吗
#   scripts/qa-run.sh          静态护栏 · 代码里的约定有没有被破坏
# =============================================================================
# ============================================================================
