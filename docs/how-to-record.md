...
**为什么这么设计**:它分析的对象首先是**余额变动**。正因为只需要几个数字,才做得到「每月 10 分钟,全家填完」。

**想要更细也可以,但那是可选项。** v1.8 起能逐笔录支出;v1.24 起支出分析成了完整一章 ——
除了「餐饮占几成」,还回答「这个月为什么多花了」和「这些钱砍不砍得掉」。
嫌一笔笔敲慢,可以直接导入支付宝 / 微信账单。**不开这一层,上面那条 10 分钟的路径原样成立。**
...
| **逐笔** | 每笔选账户 + 类目,自动从该账户余额扣除 | 上述全部 + 整章**支出分析** + 账户级收益率更准(消费不再被当成投资亏损)|

**切换不动任何已录数据**:切到逐笔后原来填的总额还在(切回来立刻生效),切回总额后已录的逐笔也不删。

逐笔支出有**两个各自独立的分类**,别混成一个:

- **资金性质**(必选)—— **日常开支 / 还贷 / 利息支出 / 转账给亲属**,内置四个。
  它决定这笔算不算「可调的那部分」:还贷和利息压不下去,日常开支才是。
- **消费分类**(可选)—— 餐饮美食 / 交通出行 / 住房物业……一棵你自己维护的树,大类下可再分细类。
  **点大类就能提交**,想细分再多点一下细类,不强制也不用预先决定。去 `/expense/categories` 维护。

另有一个勾:**「这笔是一次性的」**。买冰箱、换手机、一次旅行勾上它,这笔就不会污染「常态月均」。默认不勾。
...
切到逐笔模式后,这个框换成与收入区同构的逐笔表单(金额 + 资金性质 + 消费分类 + 支出账户),并额外解锁报表页整章「钱花在哪了」。
...
可以。先在「管理 → 提醒与模板 → 支出录入方式」切到**逐笔**,给支出选上消费分类(或用 `/expense/import` 批量导入支付宝 / 微信账单),
报表页「钱花在哪了」的**第二层**就是按消费分类的占比。除了占比,那一章还给「这个月为什么变了」(归因瀑布)和
「这些钱砍不砍得掉」(刚性 / 弹性 / 一次性三分 + 常态月均)。
