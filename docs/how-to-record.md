...
**为什么这么设计**:它分析的对象是**余额变动**,不是支出构成。它不打算告诉你"这个月餐饮占了几成"——那件事微信、支付宝、银行 App 已经做得很好了。也正因为只需要几个数字,才做得到「每月 10 分钟,全家填完」。
...
| **支出** | **一个总数**:每位成员各填一次「本月总支出」 | 支出笔数多且零散,逐笔记不现实。从微信 / 支付宝 / 信用卡账单抄一个总数过来即可 |
...
**日常消费(支出)** —— 每位成员各有一个「**本月总支出(一个数)**」的框,填一个总数就行,不用拆分类、不用逐笔。
