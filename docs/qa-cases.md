...
| v1221-BAR-LABEL-INSIDE-PROPORTIONAL-BLOCK | **把标签塞进按比例分宽的色块里,比例一小标签就被切**(2026-09-15 · v1.22.1)。报表的资产/负债对称条 `.sym-bar`,负债块宽度按占比算(这家约 14% → 390px 下只有 ~55px),而「负债 ¥1,186,700」要 85px,于是显示成「…86,700」。**修**:条本身保持比例(那是它的全部意义),标签挪到条外的下方左右两端。**判据**:凡是「块宽 = 数据比例」的可视化,标签就不能放在块内 —— 数据小的那一侧必然放不下,而那恰恰常常是更需要看清的一侧。 |
