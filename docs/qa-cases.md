...
| v121-UPSERT-IS-WHOLE-ROW | **那条通用 upsert 是「整行覆盖」,只传一列会静默抹掉另一列**(2026-09-09 · v1.21)。`PeriodMemberCashflowMapper.upsert` 的 ON DUPLICATE 写的是 `total_income_input = VALUES(total_income_input)` —— 分类填报只想动支出,复用它会把**当月收入抹成 NULL,而页面上没有任何报错**。修法:另加 `upsertExpenseOnly` 只动支出列。**这一条是 TDD 把它列进「待实测」才发现的** —— 读一眼 SQL 就够,但不列出来就不会去读。教训:凡是要复用既有写方法、而自己只关心其中一部分列时,先读那条 SQL 的 ON DUPLICATE 分支。 |
| v121-ONE-CHAR-EATS-A-ROW | **半角 ¥ 在 GBK 里不可映射,一个字符差异静默吞掉一整行**(2026-09-09 · v1.21)。`¥`(U+00A5)写进 GBK 文件再读回来变成 `?`,而 `money()` 只 replace 了 `¥` 与 `￥` → 金额解不出 → 整行 `continue`。真实 GBK 账单用全角 `￥`(U+FFE5),但既然一个字符就能悄悄吞一行,就别再枚举:改成**只保留数字/小数点/负号**的白名单。顺带把三类「说不清的行」全部改成计数披露(收支枚举不认识 / 列数超表头 / 金额解不出)。**「静默少算几行」在用户那儿表现为「导入总额比账单少一点」,基本查不出来** —— 解析器的默认行为必须是「说出来」,不是「跳过」。 |
| v121-DEPTH-IS-NOT-JUST-DISPLAY | **「深度」不只是展示概念,它同时影响映射与表单**(2026-09-09 · v1.21 · 两个同源缺陷)。类目树有两个深度(简单=只用大类 / 复杂=用到细类),TDD 把它当成纯展示。实际踩了两次:① **导入映射**只在「当前深度可填的那一层」找 → 渠道给的是**大类名**,复杂深度下全落「其他」;偏偏个别名字因为恰好有同名细类而命中,看起来像「大部分认不出来」。② 更严重:**导入的钱落在大类上,而复杂深度的表单只渲染细类的框 → 那笔钱用户看不见也改不了**。修法:映射在**整棵树**上做(细类优先、大类兜底);表单类目 = 可填的 ∪ **这一期已经有钱的**(含停用的),标「· 未细分」。**判据**:凡是「某一层数据」的概念,都要问「写入路径、读取路径、编辑路径是不是都覆盖了」——「钱在账上却没有对应的输入框」是最让人不安的状态。 |
| v121-GUARDRAIL-CRITERION-TOO-BROAD | **护栏判据写太宽,把正确的写法也报红**(2026-09-09 · v1.21 · 一版之内三次)。`NO-TEXTBLOCK-COLS` 第一版判「COLS 不许用文本块」→ 把两个本来正确的既有 mapper 报成红(它们用 `"SELECT " + COLS`,普通字符串保得住尾空格);真正的坑是**文本块以 SQL 关键字结尾再拼接**。第二版判据对了,却抓到了注释里描述这个坑的那句话,第三版才过 `codeonly`。加上 v1.20 的 `NO-PHANTOM-BTN-CLASS` 误报两次(注释里的 `btn-*`、模板自带 `<style>` 里定义的类),一共三次。**教训**:护栏写完要做两个方向的验证 —— ① 注入坏形状看它**红不红**(这个我一直做);② 确认它**不把正确写法报红**(这个我一直漏)。一条天天红的护栏等于没有护栏,还会训练人忽略它。 |
| v121-JS-LITERAL-HITS-THYMELEAF-INLINE | **JS 嵌套数组字面量 `[[...]]` 撞上 Thymeleaf 内联标记 → 响应被截断成一片空白,而 `curl` 看着完全正常**(2026-09-09 · v1.21)。`[[…]]` / `[(…)]` 是 Thymeleaf 在 `<script>` 里的表达式标记,JS 自己的 `[['a','b'], ['c','d']].forEach(…)` 恰好以 `[[` 开头 → 被当表达式解析 → **渲染期**在响应头发出**之后**炸 → 浏览器拿到 `ERR_INCOMPLETE_CHUNKED_ENCODING`、`document.body.children.length === 0`,**连错误页都没有**。最坑的是验证方式:`curl` 返回 **200 + 73KB + 新内容 grep 得到**,因为 curl 不校验 chunked 终止符 —— 「接口通了」在这类问题上是**假绿**,只有真浏览器能发现。这一条是双端截图抓到的。修法:该 `<script>` 加 `th:inline="none"`。护栏 `v1210-NO-JS-INLINE-COLLISION` + `scripts/lint/thymeleaf-inline-collision.py`。**判据注意**:合法内联不只有 `[[${…}]]`,还有 `@{}` `#{}` `*{}` `~{}` 四种(项目里 `[[@{/admin/family/logo}]]` 就是对的),第一版判据只放过 `${` 把两处正确写法报成红。 |
| v121-MYSQL8-RESERVED-ALIAS | **`COUNT(*) AS rows` 在 MySQL 8.0 是语法错**(2026-09-10 · v1.21 第 2 稿)。`rows` 是随窗口函数引入的保留字。炸的位置很讨厌:**只在真跑到那条查询时才炸** —— 编译过、启动过、单测也过(单测用内存假 mapper,根本不碰 SQL),报表页 500。同类保留字还有 `groups` / `rank` / `over` / `lead` / `lag` / `cume_dist`。护栏 `v1210-NO-RESERVED-WORD-ALIAS`。**判据**:注解里的 SQL 没有任何编译期检查,别名撞保留字这类错只能靠机器扫或者真跑一次。 |
| v121-THREPLACE-EATS-HOST-CLASS | **`th:replace` 会把宿主标签连同它的 class 一起换掉**(2026-09-10 · v1.21)。`<div class="w-full" th:replace="~{a :: b}">` 里的 `w-full` 会消失 —— replace 替换的是整个标签。宫格因此变成 `flex flex-wrap` 行里的一个普通项,挤到按钮右边、左边空一大片。改用 `th:insert`(保留宿主标签)。**功能完全正常、接口全通,只是难看** —— 这类问题只有双端截图能发现,`curl` 和单测都是绿的。 |
| v121-HIDDEN-LOSES-TO-TAILWIND | **HTML 的 `hidden` 属性打不过 Tailwind 的 `flex`/`block`/`grid`**(2026-09-10 · v1.21)。`hidden` 靠 UA 样式表的 `[hidden]{display:none}` 生效,而那条规则优先级**比任何 class 都低**。`<div hidden class="flex">` 照常显示。修法是一条全局 CSS `[hidden]{display:none!important}` 写进 `style.css` —— 而不是逐处改成 `style="display:none"`:这是**整类**问题,下一个人不会记得这条规则。 |
| v121-SWAP-WIDGET-BREAKS-ITS-READERS | **换控件形态时,坏掉的是「读这个控件的其它代码」,而且往往只在特定路径上抛**(2026-09-10 · v1.21)。类目从 `<select>` 换成按钮宫格之后,v1.19.3 的 `expense-liability.js` 还在调 `cat.options` —— 而它现在是 hidden input,strict 模式下 `Array.prototype.slice.call(undefined)` 抛 TypeError。**但只在用户选中信用卡那一刻才抛**:首屏默认现金账户走「恢复」分支提前 return,页面打开一切正常,冒烟测试和护栏全绿。**判据**:换控件形态时,先 `grep` 出所有读它的代码列成清单,而不是只看它自己渲染得对不对。 |
| v121-PREDICATE-ORDER-KILLS-A-BUCKET | **判据顺序错了会让一整个分支变成死代码,而且不报错**(2026-09-10 · v1.21)。账单分桶里「关键字划转」排在「还贷」之前,而 NEUTRAL 关键字表里正好有「还款」——于是渠道明确标成**支出**的「花呗还款」先被当划转剔掉,`NATURE` 桶**永远是空的**,确认页上那个格子形同虚设。正确顺序:**渠道明说的 → 我们能确定的 → 关键字猜的**;猜的东西不该压过渠道自己的判断。护栏 `v1210-NATURE-BEFORE-NEUTRAL` 直接比较两处判据在源码里的**行号**。**判据**:凡是「多个桶 + 逐条 if-continue」的结构,每个桶都要有一条测试真正落进去过,否则死桶看不出来。 |
| v121-SED-DELETE-BY-COMMENT-BOUNDARY | **按「往前找最近的 `/**`」批量删方法,会连坐无关代码**(2026-09-10 · v1.21)。删三个端点时连带吃掉了三个完全无关的方法(它们和被删端点共享注释块边界)。这次编译器当场报错所以没损失,但**如果被吃掉的是一个没人调用的方法,就会静默消失**。**判据**:批量删代码之后跑一遍 `git diff HEAD \| grep '^-.*\\b(private\|public)\\b.*('`,把删掉的方法名列出来看一眼。5 秒钟,挡一整类事故。 |
| v121-CHANGE-CARRIER-NOT-ALGORITHM | **换载体能消掉的复杂度,往往比在原载体上优化算法省得多**(2026-09-10 · v1.21 · 正面经验)。第 1 稿「删类目搬家」搬的是带 `UNIQUE(期,人,类目,来源)` 的汇总表,裸 UPDATE 会撞键,得先合并同槽行再删源行 —— 一段需要单测钉住的算法。改成逐笔载体之后**一条 UPDATE 就够**,因为每一笔本来就是独立一行。这不是刻意设计的,是载体换对了之后附带的。**判据**:当某个操作需要「先合并再删除」「先排序再插回原位」这类补偿逻辑时,先问一句「是不是载体选错了」。 |
| v121-T-IN-FRAGMENT-ARG | **片段表达式的参数里调 `T(...)` 静态方法会在渲染期炸**(2026-09-10 · v1.21)。`~{fragments/_cat-icons :: icon(${T(com.family...).of(name)})}` → `/entry` 从 435KB 掉到 93KB、curl 退出码 18、浏览器**永远等不到 DOMContentLoaded**(错误发生在响应头发出之后,页面截断成半张)。**判据必须窄**:普通属性里的 `T()`(`th:if`/`th:each`/`th:text`)项目里有三处、一直正常 —— 第一版判据写成「模板里不许有 T(com.family」把它们全报红了。用「注入 → 复现 → 还原」证实炸点在片段参数之后才收窄。护栏 `v1210-NO-STATIC-CALL-IN-FRAGMENT-ARG`。**顺带的分层结论**:派生值(图标 key、展示名)在 controller 算好传进模板,模板不该知道类的全限定名。 |
| v121-FEATURE-WITHOUT-ENTRY | **这一版最值钱的能力,做完了却没有入口**(2026-09-10 · v1.21 · 维护者一句「我请问了,这导入按钮在哪里??」)。账单导入只在报表页一句说明文字里内联了一个链接,**填报页(用户真正在的地方)一个都没有**,也没登记进 `scripts/entry-points.json`。我验的是「功能对不对」——逐笔归类、分桶、去重、撤销全验过——**就是没有以用户身份从顶层点一遍**(memory `feedback_verify_user_path` 写着这条)。**判据**:新能力上线前,回答「用户在哪个页面、什么心境下会想用它?那个位置有没有入口?」并登记进入口表让 `entry-points-check` 在双端验可见性。功能验证与入口验证是**两件事**,前者全绿不代表后者存在。 |
| v121-SIBLING-CELLS-SAME-SIZE | **并列的选择项必须同尺寸,层级不同才允许不同**(2026-09-10 · v1.21)。分类宫格里消费分类用 `.cat-cell`、性质项用小一圈的 `.cat-chip` —— 但它们在界面上是**并列的选择项**,不是两个层级,两排放一起高矮不齐。修:共用 `.cat-cell`,只用 `--nature` 改颜色和边框(虚线),不改尺寸。另外 grid 用 `repeat(auto-fill, minmax(96px,1fr))` 而不是 `auto-fit` —— `auto-fit` 会把只有 3 个元素那一排拉成 3 倍宽,同样是尺寸不一致。**判据**:问「这两组东西在用户眼里是不是同一层的选择?」是 → 同尺寸;不是 → 才允许不同,而且要有明确的视觉层级表达。 |
| v121-ICON-GUESSED-NOT-PICKED | **图标按名字猜,不让用户挑**(2026-09-10 · v1.21 · 正面经验)。给每个分类配图标能让宫格「一屏找一个」快很多,但让用户为每个类目挑图标,是在他本来就嫌麻烦的地方再加一步。做法:内置关键词→图标表按类目名猜,起步包大类刻意与支付宝交易分类同名所以全命中,猜不中落中性标签图标(**必须有 default 分支** —— 否则渲染成空方块,而且没人会意识到是没匹配上)。图标一律 inline SVG + `stroke=currentColor`,反色时自动变白,不需要第二套样式。 |
| v121-ENTRY-EXISTS-VS-FINDABLE | **入口存在 ≠ 入口找得到**(2026-09-11 · v1.21 · 维护者连着两轮问「导入按钮在哪」)。第一轮我漏了入口,补上之后**第二轮又被问一次** —— 那时链接确实存在,`entry-points-check` 也过了,因为**它只验「可见」不验「分量」**:一个 10px 灰色 mono 链接,挂在宫格里那条分隔线的右端、还得往下滚 250px 才进视野,几何上「可见且未被遮挡」,人却找不到。修:主入口改成支出区**标题行**上的 `btn-ghost` 按钮(33px 高、brass 描边、页面纵坐标从 1072 提到 820),下面配一句「一笔一笔记太慢?传一份月账单…」说明它能省什么事;原来那个小字降为就近的次入口。**判据**:能力入口的检查除了「渲染出来了吗」,还要问「它的视觉分量配得上这个能力吗?用户第一眼扫页面时会落在它身上吗?」—— 前者机器能验,后者只能靠人看截图。护栏 `v1210-IMPORT-ENTRY-VISIBLE` 现在同时要求按钮形态 + 说明文案。 |
| v121-BALANCE-WRITE-IS-AUTHORITATIVE | **记一笔支出会直接改写「用户自己填的期末余额」,于是「先核对余额再导账单」的人被扣第二遍**(2026-09-11 · v1.21 第四轮评审)。`applyDeltaToBalance` 写的是 `period_snapshot.end_balance` —— 那是权威值不是预填值。现有纪律「先录收支、最后核对余额」能掩盖它,但**导入天然是反顺序的**(月底打开账单页时余额多半已经核对完了),几百笔一起扣第二遍。修:加 `affects_balance` 开关。**关掉时必须三条一起退出** —— ① 不动余额 ② 不参与轧差(否则 unexplained 变负数,账户凭空出现「解释过头」)③ 不算账户外部流出(否则 NAV 以为「钱是被取走的不是亏掉的」,**把账户收益率算高而且不报错**);同时**仍要**计入家庭支出总额与支出构成 —— 钱确实花了。**做不成 `account_id=NULL`**:cash_flow 没有 family_id,家庭归属靠 JOIN account 得到,置空这笔就从家庭支出里消失了。**判据**:凡是「某个动作要变成可选」的需求,先列出这个动作实际触发了哪几条口径,**逐条决定跟不跟着关** —— 少关一条就是新的静默错误。 |
| v121-ANCHOR-MATCHES-WRONG-FORM | **用共有的字符串做锚点 + `replace(...,1)`,改到了另一个表单里**(2026-09-11 · v1.21)。`+ 加一笔` 按钮收入/支出表单各有一个,我拿它当锚点插「从余额里扣」checkbox,结果落进了**收入**表单。后果不是「少个勾选框」:支出表单没这个字段 → `@RequestParam(defaultValue="false")` → **每一笔手工支出都不再扣余额**,不报错、不提示,只是余额慢慢对不上。**双端截图抓到的** —— 编译、单测、qa-run 全绿(那时护栏判据是「页面上有 name=affectsBalance」,页面上确实有)。**两条判据**:① 插入锚点必须是**目标位置独有**的,共有字符串要配一个邻近的唯一特征(这里用宫格的 `th:insert` 那一行);② 护栏要钉到**结构层级**而不是文件层级 —— 现在断言「支出表单里恰好 1 个、收入表单里 0 个」,而不是「页面里有」。 |
| v121-ACCOUNT-PER-ROW-NOT-PER-BATCH | **一份账单跨几个账户是常态,整批落一个账户会让余额和收益率一起错**(2026-09-11 · v1.21 第五轮)。支付宝「收/付款方式」列里余额宝 / 花呗 / 招商银行储蓄卡都有;整批算到一个头上,那几个账户的余额轧差和账户级收益率全错,**而且不报错**。修:逐笔推荐(家庭规则 → 账户名互相包含**取最长匹配** → 内置别名),推荐不出来落默认账户**并标红**——账户猜错比分类猜错严重,分类错了只是构成图不准。连带:扣余额按账户分组分别扣、撤销按账户分别加回(且必须在**软删之前**统计,软删后就查不到了)。**判据**:凡是「批量导入外部数据」的功能,先问「这批数据里有哪些字段其实是逐行变化的?」—— 把变化的字段做成整批常量是最常见的一类错。 |
| v121-PREVIEW-SIMPLIFIED-AWAY | **preview 里画对的交互,实现时被自己简化掉了,中间没有任何东西拦住**(2026-09-11 · v1.21)。preview 画的是「已选 23 笔 · 批量改到分类 ▾ · 批量剔除」,实现时简化成「每行一个剔除复选框 + 每行一个下拉」——**复选框被当成了「剔除」,于是批量操作根本没有落脚点**(没有「选中」这个概念)。几百笔逐个勾等于没做。修:复选框 = 选中(纯前端),剔除用 hidden 表达由 JS 增删;全选做两级(表头 + 每个分类组一个)。**判据**:preview 是给用户看的承诺,自验时要**逐项对一遍 preview 与实现的差异**,而不是等评审来发现。差异可以有(实现时发现更好的做法),但必须是**有意识的决定**并写进实施记录。 |
| v121-DB-CHECK-SURFACED-AS-NOTHING | **DB 的 CHECK 约束撞上之后,兜底提示把原因吞了,用户再试一百次也还是失败**(2026-09-14 · v1.21 · 维护者真机导入报错)。真实账单里有**冲正行(负金额)**和占位行(0),而 `cash_flow` 上有 `CHECK(amount > 0)` → 整批事务回滚 → 用户只看到「落库失败,什么都没写进去 —— 再试一次」。我这边也只能靠翻 `/opt/finance/logs/app.log` 才知道是什么。**三处修**:① 分桶时就把金额≤0 的行挡进「已剔除」并写明原因;② 落库前再拦一道给人话;③ **兜底 catch 必须把原因翻译出来**(已知约束名 → 人话,未知 → 至少给异常类型名,并加 `log.warn(..., e)` 带堆栈)。**判据**:凡是「再试一次」这类兜底文案,先问「再试一次真的可能成功吗?」—— 如果是确定性失败,这句话就是在骗用户。 |
| v121-NATIVE-SELECT-IN-CUSTOM-UI | **新页面用了系统原生 `<select>`,和全站自研下拉不一致**(2026-09-14 · v1.21)。项目有 `data-lsel` 自研件(搜索 + 拼音全拼/首字母 + 键盘 + 短列表自动省搜索框),而我在导入页写了 7 个原生 select —— 账户和分类可能几十项,原生件既没法搜也和衬线排版格格不入。**连带一个坑**:换成自研件之后,批量赋值 `sel.value = x` **必须 `dispatchEvent(new Event('change'))`** —— 自研件的按钮文案靠 change 同步,不派发的话值是对的但**用户看到的还是旧文案**,会以为功能坏了。护栏 `v1210-NO-NATIVE-SELECT` 同时钉「没有原生 select」和「批量赋值派发 change」。 |
| v121-MEMORY-KEYWORD-MUST-BE-REUSABLE | **「记住我的改动」取商户名前 20 字当关键字 —— 规则越攒越多却一条都不复用**(2026-09-14 · v1.21)。「瑞幸咖啡(国贸店)」前 20 字仍带门店名,换一家店就不命中。改成剥掉**括号内容 / 末尾编号 / 企业后缀**,剥过头(≤1 字)退回原名 —— 一个字的关键字会把半个账单都匹配进去。配 3 条单测钉住。**另一半**:只写不给看的记忆是不完整的,用户第一次发现它归错类时无从下手 —— 所以两类规则(商户→分类、资金来源→账户)都要能**看见和删除**。 |
| v121-SHARED-RECORD-ONE-QUERY | **一个 record 被两条查询共用,只给其中一条加列 → 运行期 IndexOutOfBounds**(2026-09-14 · v1.21)。`IncomeEntryRow` 同时是 `findIncomeEntries` 和 `findExpenseEntries` 的返回类型。我给支出那条加了 `expenseCategoryName`(第 12 个组件),收入那条仍返回 11 列 → MyBatis 构造 record 时 `Index 11 out of bounds for length 11` → **填报页整页变成错误页**。**编译过、单测过**(单测不碰真 SQL),只有真打开页面才炸。修:收入那条也给这一列(`NULL AS expenseCategoryName`,收入本来就不接消费分类)。护栏 `v1210-SHARED-RECORD-BOTH-QUERIES` 断言新列在两条查询里各出现一次。**判据**:给 record/DTO 加字段前,先 `grep` 它被哪些查询用作返回类型 —— 注解式 SQL 没有任何编译期检查。 |
| v1222-PREDICATE-SCANS-NATURAL-LANGUAGE | **判据禁了一个中文词,而那个词必然出现在解释这条规则的注释里 —— 同一个坑的第六次**(2026-09-16 · v1.22.2)。`v1222-BOTTOM-PAGER` 要确认「没改成无限滚动」,第一版扫 `IntersectionObserver|infinite|无限滚动`,而上面那段注释正好在写「【为什么不做无限滚动】…」。前五次分别栽在 `已剔除` / `金额是 0 或负数` / `applyImportedExpense` / `max-w-[120px]` / 跨行的 Thymeleaf 注释上。**这次的教训比「先剥注释」更进一步**:剥注释治标,根子在于**判据不该扫自然语言**。代码标识符(`IntersectionObserver`、`scrollend`)不会出现在解释性文字里,中文词一定会 —— 因为你越是把规则解释清楚,就越会写下那个被禁的词。**判据**:写「不许出现 X」时,X 必须是**代码里的标识符**,不是概念名。 |
| v1222-PREDICATE-TOO-LOOSE-TO-CATCH-VALUE | **判据只检查了「结构在不在」,值改错了照样绿**(2026-09-16 · v1.22.2)。`v1222-MOBILE-PAGE-SIZE-5` 查了 `defaultSize()` 存在、`matchMedia` 存在、下拉同步存在、档位有 5 —— 但没查**手机分支到底返回几**。注入 `? 10 : 10` 之后它仍然 PASS。**判据**:护栏要钉的是**需求本身**,不是实现的骨架。「手机端 5 条」这个 5 是用户明确要的,就该绑上去 —— 这和「不许绑偶然的字面量」不冲突:**需求里的数字要绑,实现细节里的数字不要绑**。 |
| v1222-SET-U-KILLS-THE-REST | **护栏脚本里一行手滑的变量引用,把它之后的所有护栏全部中断**(2026-09-16 · v1.22.2)。写新护栏时留了一行 `grep -q ... "$QA1221_FLOWJS_OR_NEW"`,那个变量根本不存在,而脚本开了 `set -u` → 整个 qa-run 在那里退出,后面几十条护栏一条没跑,而**总结行还打印出来了**,看着像正常结束。**判据**:改完 qa-run 之后要看 **exit code 和 PASS 总数**,不能只看自己新加的那几条是不是绿的 —— 总数突然变小就是有东西被中断了。 |
| v1224-PAGER-SCROLLED-THE-PAGE-AWAY | **翻页时把页面滚走 —— 而用户的视线本来就在他刚点的按钮上**(2026-09-17 · v1.22.4 · 维护者「点击下一页,为什么刷新后整体页面位置还有移动?要保持页面不动,只是翻页」)。底部翻页按钮点一下就 `list.scrollIntoView({block:'start'})`,实测跳 **172px**。当时的想法是「翻完页回到列表顶端」,但那是**加载新页面**的心智,不是**换一屏内容**的心智。**修**:锚定按钮 —— 记下它翻页前的视口位置,`render()` 之后用 `scrollBy` 补回差值。**一个必须承认的取舍**:从末页(1 行)退回满页(5 行)时列表高度必然增加约 297px,这时 `scrollY` 和「按钮不动」二选一 —— 选了后者,因为让 `scrollY` 不动会把新长出的行推着按钮跑出视口,用户就点不到下一页了。**判据**:分页控件旁边的内容变高变矮时,该钉住的是**用户刚交互的那个元素**,不是 scrollY 的数值。 |
| v1224-SEVENTH-TIME-COMMENT-TRIPPED-PREDICATE | **判据第七次被自己的注释绊倒,而且是在我刚总结完前六次之后**(2026-09-17 · v1.22.4)。`v1224-PAGER-NO-JUMP` 扫「代码里不许有 `scrollIntoView`」,而它自己的注释里写着「不用 scrollIntoView」和「第一版这里是 `list.scrollIntoView(...)`」。**上一轮(v1.22.2)我刚得出结论「判据要扫代码标识符、不要扫自然语言概念名」—— 这次证明那还不够**:标识符一样会被写进解释里。真正的根子是**扫描范围含注释**。`qa-run` 的 `codeonly` 只剥 Java 的 `//`,对 JS 的 `/* */` 块注释无效,所以 JS 判据要单独剥。**判据**:任何「不许出现 X」的护栏,**第一件事**是把扫描范围限定到剥过注释的代码,而不是先写判据、等它红了再补。 |
| v1224-PREDICATE-CAUGHT-PAGE-ARITHMETIC | **「不许做金额运算」的判据用裸 `+=` 去抓,结果抓到了 `page += delta`**(2026-09-17 · v1.22.4)。`v1212-FLOW-TOTALS-NOT-FILTERED` 扫 `parseFloat|Number(|toFixed|+=`,而翻页锚定里的 `page += delta` 是**页码**运算,和钱毫无关系 —— 判据红在了一个完全正确的实现上。收窄成 `(amt|sum|total|money|balance)\w*\s*\+=`。**判据**:用一个通用的语法符号(`+=`、`*`、`/`)去代表一个**语义**概念(金额运算),迟早会抓到同形状但无关的代码。抓语义就得带上语义线索(变量名前缀),或者换个只在那个语义下才出现的标识符。 |
| v1224-SSR-LIST-NAKED-UNTIL-JS | **服务端全量渲染 + JS 分页 = 首屏铺开整月流水几秒钟**(2026-09-17 · v1.22.4 · 维护者「第一次进入填报页还是先展示了巨大的流水列表,然后隔了一会才变成分页形式」)。前端分页的前提就是把 201 行全量渲染进 DOM,而 `flow-table.js` 要等 `DOMContentLoaded` 才接管 —— **实测裸奔 2.9 秒**:先铺开一条望不到头的长龙,过几秒突然缩成 10 行。**修**:CSS 在 head 里、阻塞渲染,比任何 JS 都早 —— 用 `.flow-prepaged > [data-flow-row]:nth-of-type(n+11)` 先藏掉多余的行,JS 接管时 `classList.remove` 摘掉,之后照旧由 `hidden` 控制。阈值要和 JS 的 `MIN_ROWS` 一致(护栏钉着),否则会出现「CSS 藏了但 JS 不接管」——那些行就永远看不见了。**判据**:凡是「服务端渲染全量 + JS 收敛」的列表,都要问一句「JS 到之前用户看到什么」。 |
| v1224-MY-OWN-IMPORTANT-BROKE-IT | **自己几个版本前加的 `!important` 把新规则盖掉了,而且只在手机端**(2026-09-17 · v1.22.4)。v1.22.1 给手机端加过 `[data-flow-row]:not([hidden]) { display: flex !important; }`(把流水行改成 flex 排布)。而 **JS 没跑时这些行还没有 `hidden` 属性**,`:not([hidden])` 全部命中 → 那条 `!important` 把 v1.22.4 预分页的 `display:none` 整个盖掉。PC 端没有那条规则,所以 **PC 正好 10 行、手机 30 行全露**。**更麻烦的是它极难测出来**:前几次验证都因为浏览器缓存让 `flow-table.js` 已经跑过(`route` 只拦网络请求,缓存命中根本不发请求;CDP 禁缓存也没挡住),一直显示「正常」——最后用 `javaScriptEnabled:false` 的独立上下文(登录后搬 cookie 过去)才看见真相。**判据**:① 要验「JS 之前的样子」,唯一可靠的办法是**彻底禁用 JS**,不要试图延迟/拦截脚本;② 自己写下的每一个 `!important` 都是给未来的自己埋的雷,尤其是带 `:not([某个由 JS 设置的属性])` 的那种 —— 它在 JS 跑之前的命中范围和之后完全不同。 |
| v1224-STREAM-ORDER-NOT-VISUAL-ORDER | **`order: -1` 改得了视觉位置,改不了到达顺序**(2026-09-17 · v1.22.4 · 维护者「目录要等好多秒才展示」)。长文目录排在主内容**之后**、靠 `order:-1` 视觉上放到左列,注释写的理由是「DOM 仍内容优先」。但 HTML 是**流式到达**的:报表页几十个 section,实测目录比主内容晚 **0.5~1 秒**才出现。**修**:把目录移到 DOM 最前,去掉 `order:-1` —— 实测差值降到 **0ms**。顺带这对可访问性也更好:目录是**导航**,先到达意味着屏幕阅读器用户可以直接跳到想去的节,而不是先听完几十个 section。**判据**:CSS 的 `order` / `grid-area` 只影响绘制,不影响字节到达的先后。凡是「这个元素出现得太晚」,先看它在 DOM 里排第几,而不是找 JS。 |
| v1222-FLEX-BASIS-TIPPING-POINT | **靠 flex-basis 的临界点去「刚好换行」,两次都翻车,而且截图上看不出来**(2026-09-16 · v1.22.2)。账户卡要「第一行账户名、第二行余额+刷新」:① `basis: calc(100% - 40px)` —— 加上容器 8px 的 gap,总和刚好超 4px,刷新按钮掉到第三行;② 改成 `basis: 60%` —— 又变成整行都塞得下,于是账户名(`flex: 1 1 0`)只分到 **15px**,被压成一个省略号。**两次在截图上都不明显**,得量 `clientWidth` 才看得出来。**修**:用 flexbox 的标准断行 —— `order` 分组 + 一个 `flex-basis:100%` 的零高伪元素,没有临界点。**判据**:想让 flex 在某处换行时,不要去算「让它刚好放不下」;那是在跟 gap、border、字体渲染的舍入较劲。用 order + 断行伪元素表达意图。 |
| v1222-GUARDRAIL-HARDCODED-COUNT-AGAIN | **护栏又写死了出现次数,又漏了第三处**(2026-09-16 · v1.22.2)。`v1222-PREV-BALANCE-ONCE` 判 `grep -c previousBalanceLabel == 2`,而实际有 3 处 —— 第三处在折叠的「改本期余额」表单里当参考值,**那是另一个上下文、完全合理**。同一个坑 v1.21.2 刚踩过(`-eq 2` 把第三条查询漏掉)。**判据**:护栏里出现字面数字就该警觉 —— 它把「当下的事实」冻成了「永远的约束」。改成钉**实际的修复点**(那份有 `acct-prev` 标记且手机端收起、右侧仍保一份),不数总数。 |
| v1222-ICON-WITHOUT-AFFORDANCE | **一个长得像被选中的复选框、但既点不动也没有 title 的图标**(2026-09-16 · v1.22.2 · 维护者第一句话就是「这个 icon 是干嘛的?」)。`tick-done` 表示「本期已填」,是纯展示,但它的视觉语言(方框 + 勾 + 绿色)在所有界面里都意味着「可点击的、已选中的复选框」。用户点不动,也没有任何文字说明。**修**:补 `title` + `aria-label`;同时把下面那行重复的「已填/待填」在手机端收起 —— 信息由这个勾承担,不用说两遍。**判据**:借用了某个控件的视觉语言,就得给出那个控件的可供性,或者给出说明;两样都不给,用户只能猜。 |
| v1221-TWO-SIGN-CONVENTIONS | **同一个页面上两套负号写法并存,用户以为是两种不同的东西**(2026-09-15 · v1.22.1)。`MoneyFormat` 走 `sign + symbol + abs` → 「−¥12,345」;而模板与 JS 里散落的 `symbol + format(amount)` 在负数时拼成「**¥-12,345**」。两者在仪表盘同屏出现。修:加 `MoneyFormat.intSigned(symbol, amount)`,把模板里 20+ 处 `currencySymbol + #numbers.formatInteger(...)` 和 JS 的 `fmtMoney` 统一收口。**判据**:格式化「符号 + 数字」时,负号必须先于符号取出来 —— `NumberFormat` 自带的负号总是紧贴数字,拼在货币符号后面就变成了另一套约定。护栏 `v1221-SIGN-BEFORE-SYMBOL`。 |
| v1221-GUARDRAIL-BANNED-CORRECT-CODE | **护栏判据写太宽,红在一个正确的实现上**(2026-09-15 · v1.22.1)。`v1221-SIGN-BEFORE-SYMBOL` 第一版禁止所有 `currencySymbol + #numbers.formatInteger`,但 `(wf.investPnl().signum() >= 0 ? '+' : '−') + currencySymbol + ...` **本来就是对的** —— 它自己先处理了符号。收窄成「只禁 currencySymbol 出现在表达式开头」。**判据**:写「不许出现 X」之前先跑一遍看有多少现存命中;命中里只要有一个是正确实现,判据就该按**位置/上下文**收窄,而不是靠白名单逐个豁免。 |
| v1221-BAR-LABEL-INSIDE-PROPORTIONAL-BLOCK | **把标签塞进按比例分宽的色块里,比例一小标签就被切**(2026-09-15 · v1.22.1)。报表的资产/负债对称条 `.sym-bar`,负债块宽度按占比算(这家约 14% → 390px 下只有 ~55px),而「负债 ¥98,765」要 85px,于是显示成「…86,700」。**修**:条本身保持比例(那是它的全部意义),标签挪到条外的下方左右两端。**判据**:凡是「块宽 = 数据比例」的可视化,标签就不能放在块内 —— 数据小的那一侧必然放不下,而那恰恰常常是更需要看清的一侧。 |
| v1221-PC-SIZES-DONT-SURVIVE-390PX | **全站 tag/pill 只按 PC 调过,搬到 390px 上会把核心信息挤掉**(2026-09-15 · v1.22.1 · 维护者「非常不美观, 易用性和可用性都极差」)。填报页流水行第一行是 `grid-cols-[auto_auto_1fr_auto]`(4 列),而有消费分类时这一行有**5 个**元素 —— 第 5 个(金额)自动落到下一行,孤零零掉在左下角。同时账户名是唯一可收缩的元素,被压到带省略号。**这不是宽度问题,是列数不够 + 尺寸没收口**。修:手机端改 flex、账户名 `flex:0 0 auto` 不参与压缩、该让路的是消费分类、pill 字距归零(中文不需要 0.1em)。**判据**:`letter-spacing` + `text-transform:uppercase` 对中文只增加宽度不增加可读性;一行元素 ≥4 个时,先问「哪个是主语」,让别的给它让路。 |
| v1221-IMPORTANT-OVERRODE-HIDDEN | **给手机端写 `display:flex !important`,把全局的 `[hidden]{display:none!important}` 盖掉了**(2026-09-15 · v1.22.1)。两条都是 `!important`、同特异性,后写的赢 —— 而我的手机端样式在文件末尾。后果:前端分页藏起来的几百行被重新放出来,列表炸成十几屏,且那些行的布局是塌的(账户名宽度只剩 2px)。**判据**:任何 `display:*!important` 都要带 `:not([hidden])`;改完之后**量一次可见行数**,不能只看截图第一屏。 |
| v1221-CORE-ELEMENT-HARD-CLAMPED | **核心元素被硬编码 max-width 锁死,而同行的次要元素全是 flex-shrink-0**(2026-09-15 · v1.22.1)。`entry/_row.html` 给账户名写死 `max-w-[120px]`,同行的「本月已校准」badge / 类型 / 币种都不收缩 —— 账户名成了唯一能被压的那个,实测在 390px 下被压到 **2px**,「招行理财-稳健」只剩一个省略号。**判据**:给元素加 `max-w-[Npx]` 之前先问「这一行里它是主语还是配角」;主语不设上限,让配角截断。护栏 `v1221-ACCOUNT-NAME-NOT-CLAMPED`。 |
| v1221-FOURTH-TIME-COMMENT-TRIPPED-GUARDRAIL | **判据被自己的注释绊倒 —— 这是同一个坑的第四次**(2026-09-15 · v1.22.1)。`v1221-ACCOUNT-NAME-NOT-CLAMPED` 扫「不许出现 `max-w-[120px]`」,而它的注释里就写着「账户名 max-w-[120px]」。前三次见 `tech-design/v1.22.md` §五。**我在那份文档里刚写下这条教训,下一轮就又踩了。**说明「记在文档里」不够,得让判据本身默认剥注释。**判据**:写「不许出现 X」的护栏时,**第一件事**就是套上剥注释的外壳,而不是等它红了再补。 |
| v1220-VERB-IMPLIES-WHO-DECIDES | **「已剔除」这个词描述的是系统已经执行完的动作 —— 而做那个判断的依据是启发式的**(2026-09-15 · v1.22 · 维护者原话「你直接做主删掉的? 这个是不不合适」)。剔除的判据(退款/划转/交易关闭/关键字)会错,而「剔错了」的后果是**支出少算**、用户不会立刻发现:**一个不可靠的判断配了一个最不可逆的动词**。更糟的是金额 ≤ 0 的行写着「不能捞」——用户被单方面否决,还不给理由(理由其实是 DB 的 `CHECK(amount > 0)`,他看不到)。**修**:每行一个「录入」勾,系统只给**默认值 + 逐行理由**;没勾的叫「未录入」(结果状态,不是操作)。**判据**:动词的时态就是权力的归属。「已剔除 / 已删除」= 我做完了;「不建议 / 默认没勾」= 你来定。**凡是系统的判断可能错、而错的代价由用户承担,动词就必须是可否决的那一种。** |
| v1220-ABS-IS-WORSE-THAN-REJECT | **取绝对值比直接拒收更糟,因为它无声**(2026-09-15 · v1.22)。`EntryService.positiveMoney` 里有 `value.abs()`,于是一笔 −499 的退款冲正被**静默记成 +499 的消费** —— 不报错、不提示,当月支出凭空多出一千块。同一个方法里的 `signum() <= 0 → 拒收` 反而是安全的(用户会看到错误)。**判据**:处理「不该出现的值」时,**改写**比**拒绝**危险得多 —— 拒绝会被发现,改写不会。规范化函数里出现 `abs()` / `max(0,x)` / 默认值兜底时,先问一句「如果输入本来就是合法的另一种情况,这行代码会不会把它悄悄改成别的东西」。 |
| v1220-ZERO-MEANS-NO-DATA | **把「≤ 0」当成「没有数据」,会让整段内容凭空消失**(2026-09-15 · v1.22)。放开负数之后扫出 5 处,最危险的是 `Composition.hasData()` 判 `totalBase.signum() > 0` —— 某月退款多于消费 → 总额 ≤ 0 → **整个「支出构成」段被判成「没数据」直接隐藏**,不报错、不解释,而那个月明明有几十笔支出。**修**:有没有数据看**有没有行**(`!slices.isEmpty()`),不看金额符号。**但有一处故意保留**:`decide()` 的 `hasItem` 仍判 `signum() > 0`,它判的不是「有没有数据」而是「逐笔和手填总额谁说了算」,一行 0 元顶掉总额会让支出凭空变 0(单测钉着)。**判据**:`signum() > 0` 出现在「有没有」的判断里就是可疑的;但改之前要分清它守的到底是**存在性**还是**优先级** —— 后者错的代价在口径层,比前者大。 |
| v1220-GUARDRAIL-TRIPPED-BY-ITS-OWN-COMMENT | **护栏扫「不许出现某句话」时,解释这条规则的注释本身会让它变红**(2026-09-15 · v1.22 · 一轮里踩了三次)。`v1220-NO-DELETE-WORDING` 被注释里的「『已剔除』这个说法没了」绊倒;`v1220-PER-ROW-REASON` 被「『金额是 0 或负数』这种合并说法对用户没用」绊倒;`v1220-ACCOUNT-MOVE-BOTH-SIDES` 因为注释里提到 `applyImportedExpense` 把调用计数从 2 变成 3。另外 **Thymeleaf 的注释是 `<!--/* ... */-->` 跨行的**,单行 `sed 's/<!--.*//'` 只能吃掉起始那一行。**判据**:凡是判据形如「不许出现 X」,**先 codeonly / 先按块剥注释**——否则你越把规则解释清楚,护栏越红。 |
| v1220-FIXTURE-THAT-MOVES-MONEY-GOES-LAST | **会改余额链的 e2e 夹具必须排在所有主线之后**(2026-09-15 · v1.22)。「改账户挪余额」那个块起初放在主线 12 里,清理时把当期余额加回 700 —— 但 `creditAccountBalance` 会**顺延影响后续账期的期初**,补不干净,于是「超级 Agent 看到的总资产」红了 0.04,看起来像产品 bug。**判据**:夹具的副作用如果跨越了「当前这一条断言」的范围(改余额链、改账期状态、改家庭配置),就别指望能精确还原 —— 排到最后去。查这类红灯时先问「我自己造的数据有没有漏出来」,再怀疑产品。 |
| v1212-SHARED-RECORD-THIRD-QUERY | **共用 record 的护栏把「有几条查询」写死成 2,于是第三条漏了列、护栏一路绿、抽屉在 prod 上一直是空的**(2026-09-14 · v1.21.2 发现,bug 随 v1.21.0 上线)。`IncomeEntryRow` 实际被**三条**查询共用:`findIncomeEntries` / `findExpenseEntries` / **`expenseBreakdownDetail`**(报表 → 支出构成 → 逐笔抽屉)。v1.21 给 record 加了 `occurredAt` + `expenseCategoryName`,只补了前两条;第三条仍返回 11 列 → MyBatis 构造 record 时 `Index 11 out of bounds` → 抽屉空。**为什么一直没人发现**:它走 HTMX 片段,失败只是**没内容**,不报错、不变错误页,肉眼看着就像「这一组本来就没数据」。**修**:第三条补齐两列(细类带父名,与填报页同口径);护栏判据改成**「用到这个 record 的查询有几条,新列就得出现几次」**,不写死数字。**判据**:护栏里出现字面数字(`-eq 2`)时就该警觉 —— 它把「当下的事实」冻成了「永远的约束」,而代码会长。 |
| v1212-E2E-LAGS-BEHIND-NEW-PARAM | **接口新增了显式参数,e2e 夹具没跟上 → 4 条主线红,看着像回归其实是夹具旧了**(2026-09-14 · v1.21.2)。v1.21 把「从余额里扣」做成可选参数(页面复选框默认勾着),而 e2e 的 `POST /entry/expense` 不带 `affectsBalance` → 服务端 `defaultValue=false` → 余额一分不动,连带「负债方向」「删除冲回」一起红。**先用真浏览器确认线上路径没坏**(表单确实带 `affectsBalance=true`)再改夹具 —— 顺序反了就会去「修」一个没坏的东西。**不许把服务端默认改成 true**:未勾选的 checkbox 本来就不提交值,默认 true 会让「不扣余额」这个选项彻底失效。**顺带补一条反向断言**(不带参数时余额必须不动):少了它,把默认改成 true 也照样全绿。 |
| v1212-WAITING-IS-THE-WRONG-QUESTION | **给了两个「怎么等得舒服」的方案,正确答案是「为什么要等」**(2026-09-14 · v1.21.2)。上传阶段同步调 AI → 页面停在「读到 428 笔」几十秒无反馈,用户以为死机。候选 a(loading 页仍强制等)只是把等待包装好看;候选 b(后台跑 + 轮询逐行回填)要处理「AI 回来时用户已改过这行」的**竞态**,而那是正确性问题不是体验问题。实际做法:**上传阶段根本不调 AI** —— 前两层归类是纯本地的,确认页本该立刻出来;AI 降级成确认页上的一个按钮,结果**在客户端应用**(确认页本来就逐行提交分类值),于是竞态**根本不存在**。手改过的行打 `dataset.touched`,AI 永不覆盖。**判据**:遇到「怎么让等待可接受」,先问「这个等待是必须的吗」——两个候选都在优化等待时,往往说明前提没被质疑。 |
| v1212-HANDLER-SCOPE-SILENT-NULL | **事件绑定的查找作用域错了,表现是「点了没反应、也不报错」**(2026-09-14 · v1.21.2)。分桶页签在 `<form>` 之**外**(它是总览,不是要提交的字段),JS 里写 `form.querySelector('[data-bucket-tabs]')` 拿到 `null` → 整段绑定静默跳过。**页面长得完全正常**,curl 200、双端截图也看不出来,只有真机点一下才暴露。同一个 init 里的 AI 按钮也栽在这:它在 `[data-bill-bar]` 的**兄弟**节点里,`bar.querySelector` 同样拿到 null。**判据**:凡是「绑定了但没生效」这一类,先把 `querySelector` 的**容器**打出来看是不是 null;写护栏钉死查找作用域(`v1212-BUCKET-TABS-DOCUMENT-SCOPE`)。 |
| v1212-CSRF-HEADER-NAME-GUESSED | **fetch 自己拼 CSRF header 名 → 稳定 403,而前端报的是「网络没通」**(2026-09-14 · v1.21.2)。本项目配 `CookieCsrfTokenRepository`,它认的 header 是 `X-XSRF-TOKEN`;手写 `X-CSRF-TOKEN` 每次都 403 → `r.json()` 解析 HTML 失败 → 掉进 `.catch` → 文案「网络没通,稍后再试」。**把 403 说成网络问题,用户会一直重试一个永远不会好的东西。**修:token 当**表单参数**发,参数名从 hidden 域的 `name` 上读,不依赖任何 header 命名约定;`.catch` 文案带上真实原因(`HTTP 403` / 异常 message),非 2xx 单独抛。**判据**:兜底文案不许猜原因;`fetch` 的 `!r.ok` 必须单独处理,否则所有非 2xx 都会被报成网络故障。 |
| v1212-GUARDRAIL-FALSE-GREEN-EMPTY-EXTRACT | **护栏用 awk 抽方法体,pattern 差一个字符 → 抽出空串 → grep 找不到东西 → 常绿**(2026-09-14 · v1.21.2)。`awk '/ Draft toDraft\(/'` 要求 `Draft` 前是空格,而签名是 `BillCategoryResolver.Draft toDraft(`,前面是**点**。这条护栏在「什么都没检查」的状态下一直 PASS,注入回归形态时才发现它不红。**修**:所有 awk 抽块的判据先断言 `[ -n "$blk" ]` —— **抽不出来 = 判据坏了,要红**。**判据**:新护栏一律**两个方向都验** —— 正确形态要绿 **且** 注入错误形态要红;只验前者等于没验。 |
| v1212-FRONTEND-GUARD-MISSED-A-PATH | **提交拦截只数了一种「有内容」,另一条真实路径被自己拦死**(2026-09-14 · v1.21.2)。拦截逻辑是「没有未剔除的行就不让提交」,但第二次导同一份账单时**要导入 0 笔、全是已存在**,用户想把误剔的几笔「捞回来」——`restore` 勾了却仍被拦住,弹的还是「所有笔都被剔除了」,和他正在做的事完全对不上。**修**:`restore:checked` 也算有内容;两种空状态给不同文案。**这条是真机走完整往返(导入→提交→再导同一份→捞回来)才发现的**,静态读代码和单测都看不出来。**判据**:前端拦截要把**所有会产生提交内容的控件**都数进去,不能只数主列表。 |
| v121-LIST-SHOWS-WRONG-DATE | **列表显示「填报时间」,导入进来的几百笔全是同一个时刻**(2026-09-14 · v1.21)。手工记一笔时填报时间≈交易时间,差别看不出来;而导入的 26 笔全在同一秒入库,按日期搜「09-1」直接全中 —— 搜索形同虚设。账单里本来就有真实交易日(`occurred_at`),拿来显示和搜索,填报时间退到 title 里。**判据**:一个字段在「手工逐条」场景下看起来够用,不代表在「批量导入」场景下够用 —— 引入批量入口时,回头看一遍所有「按时间」的展示与筛选。 |
| v1230-SILENT-WRONG-PERIOD | **`findCurrentOpen` 在双 OPEN 下静默返回最新那期 —— 不报错、不进日志,只给一个看起来合理但是错的数**(2026-09-18 · v1.23 · issue #20)。SQL 是 `WHERE status='OPEN' ORDER BY period_start DESC LIMIT 1`,全仓 **30 处**调用点都隐含「只有一个 OPEN」。真正算错的有三处:`recentClosed` 只剔**一个** `inProgress` id → 半填的补录期被当已关账**算进月均支出**(月均变低 → 紧急储备虚高 → FIRE 目标跟着错);`ReportReminderScheduler` 只对最新期算 `daysLeft` → 补录期宽限剩 1 天**一条提醒都不发**;`resolveAnchor` 锚「最近已关账」→ 宽限期内报表**倒退回上上期**。**修**:按语义拆成 `findBalancePeriod`(余额轴,只有一个「现在」)与 `findRecordableOpen`(收支轴,可以有两期),旧方法标 `@Deprecated` + 护栏拦调用数上涨。**判据**:一个返回「当前 X」的查询,如果它靠 `ORDER BY ... LIMIT 1` 来消除歧义,那它在「X 可能有多个」的那天就会静默选错 —— 而**名字里没有任何东西提示你它做了这个选择**。 |
| v1230-DUAL-OPEN-WAS-ALREADY-REACHABLE | **以为是新引入的状态,其实早就可达,只是没人走过那条路**(2026-09-18 · v1.23)。设计双活跃账期时把「两期同时 OPEN」当成本版新增的风险;实际上**管理页「重开账期」一直就能造出它**(重开一个已关的期,它和当期就同时 OPEN)。也就是说上面那三处算错**在 v1.23 之前就存在**,只是那条路径少有人走。**判据**:评估「新状态带来什么风险」之前,先搜一遍**现有代码里有没有别的路径也能到达这个状态** —— 如果有,那就不是新风险,是一直没被系统性检查的既有缺口,优先级应当更高而不是更低。 |
| v1230-SETTLED-NOT-CLOSED | **判据要问「填完了没有」,不是「关了没有」**(2026-09-18 · v1.23 §4.4)。`FactSlice` 的收益类锚点只认 `status='CLOSED'`,双活跃下补录期没关 → 本月资产收益 / XIRR / TWR / 基准对比 / 人赚钱赚 / 账户表现**集体锚回上上期**,报表看起来「少了一个月」,而 dashboard 一起退。但**不能简单改成「在宽限里就认」**:用户压根没填时补录期就是真的半填,锚它会重演 v1.6.30 那次「未录收支全算成投资收益」。正解是「CLOSED **或** 已自然结束**且填报完成**」,后者用现成判据(无 PENDING todo + 全员已 complete,与 `markCompletedByMember` 触发自动关账同源)。**顺带修了一个既有缺陷**:填完但还没到 00:30 的期同样不被采纳,只是那个窗口只有几小时没人注意。**判据**:状态位(CLOSED)是**结果**,别拿它当**事实**(数据齐不齐)的代理 —— 两者平时高度相关,一旦时序被拉开就会分岔。 |
| v1230-JUDGE-IN-THE-WRONG-LAYER | **把「已定稿」的判据写进事实层 SQL,撞上另一条更老的纪律**(2026-09-18 · v1.23)。判据要数「活跃成员」(`member.archived_at IS NULL`),第一版直接加在 `FactMapper.xml` 里 —— 立刻被 `v115-NO-MEMBER-ARCHIVE-IN-SUMS` 判红:事实层的金额口径不许按成员归档过滤(归档只影响「谁还来填报」,不影响「家里有多少钱」)。**语义上我问的确实是「谁还来填报」**,但它住错了地方。**修**:整条查询搬到 `PeriodMapper`(账期生命周期),事实层一个字不碰;顺带让「已定稿判据」只剩一处 SQL 家族,两份(取最新一个 / 取窗口内全部)判据逐条相同并落护栏。**判据**:护栏红了先别急着收窄判据 —— 先问「它是不是在告诉我这段代码放错了层」。这次如果去改护栏,就会把一条守了很久的纪律凿出一个洞。 |
| v1230-CTOR-CHANGE-RIPPLES-TO-TESTS | **给 service 加一个依赖,9 个测试文件同时炸**(2026-09-18 · v1.23)。`FactViewServiceImpl` 用 `@RequiredArgsConstructor`,测试里全是手写 `new FactViewServiceImpl(a, b, c, ...)` 的长参数列表 —— 加一个字段,所有调用点参数错位/数量不符。而且**字段插在中间**时,有些测试的错误是运行期 `NoSuchMethodError` 而不是编译失败,更难定位。**判据**:改 `@RequiredArgsConstructor` 类的字段前,先 `grep -rn "new <类名>(" src/test` 看有多少手写构造点;数量大时把新依赖**加在字段列表末尾**(至少让编译错误集中在「少一个参数」这一种形态)。 |
| v1230-E2E-WASNT-E2E | **叫了两年 e2e 的脚本,其实是数据层回归**(2026-09-20 · 维护者指出)。`scripts/e2e.sh` 1146 行、22 主线、**75 处 curl + 119 处 SQL、零浏览器** —— 它验「接口通了、库里算对了」,而把用户与系统之间的一整段跳过了:表单字段名、按钮可达性、JS 绑定作用域、渲染完整性、并列元素尺寸。**这个项目的事故恰恰大量出在那一段**:分桶页签绑错作用域点了没反应(v1.21.2)、chunked 渲染中途炸掉 curl 照报 200(v1.19)、账户名被 `max-w` 压成 2px(v1.22.1)。**处置**:新建 `scripts/e2e/`(Playwright 驱动,动作一律从页面元素发起),旧脚本**正名**为 `scripts/regression-data.sh` 而不是删掉 —— 它那 119 处 SQL 验的是**计算口径**(XIRR 币种无关、精确算术、归因锚点),那些本来就没有 UI 路径,硬塞进浏览器只会更慢更脆。**判据**:一个测试叫什么名字,决定了别人以为哪些风险已经被覆盖了。名不副实的绿灯比红灯危险。 |
| v1230-E2E-FOUND-CUSTOM-WIDGET | **浏览器 e2e 第一次跑就抓到 curl 版结构性抓不到的东西**(2026-09-20)。填报页的账户选择器是自研件(`<select data-lsel>` 被隐藏,另造 `.lsel-btn` + `.lsel-panel` 顶上去)。curl 直接 `POST accountId=25` **永远成功**,而用户点的是完全另一套 DOM。第一版 flow 用 `selectOption` 直接超时 —— 那正是它该报的错。**修**:`pickLsel` 走真实路径(点按钮展开 → 点列表项),**并回头断言原生 select 的 value 真的变了** —— 最后那步专门守「组件赋值但没 `dispatch('change')`」那类 bug,只验「点得动」抓不到它。**判据**:凡是被 JS 接管的原生控件,e2e 必须走组件的交互路径;用 `{force:true}` 去操作隐藏的原生元素虽然能过,但那测的是一条用户永远走不到的路。 |
| v1230-E2E-PAGINATION-BLIND | **e2e 假设目标在第一页,而列表是倒序分页的**(2026-09-20)。还原现场时要点某一期的「关闭账期」,`form[action*="/162/force-close"]` 数量恒为 0 —— 因为周期管理按 `period_start DESC` 分页,而 beta 的账期表**预建到了 2041**,前 24 项全是未来期,2026-08 在第二页往后。**修**:加 `paginateUntil(pathTpl, selector)`,像用户一样翻页找。**判据**:凡是在分页列表里找东西的 e2e,都要问一句「它一定在第一页吗」——「按时间倒序」+「预建未来数据」是个很常见的组合,而它让第一页完全没有你要的东西。 |
| v1230-QA-RUN-ORDER-COUPLING | **两条护栏红了 40 分钟,根因是 qa-run 自己改了配置还没还原**(2026-09-20)。`v04-AI-REBALANCE-5/7` 报「刷新按钮缺」,而手工取同一个页面**按钮明明在**(`refresh=true` 1 处、`↻ 刷新` 2 处)。查下去:AI 建议缓存按 `family_id + anchor_code` 存,而 qa-run 里有 **11 处**护栏会改 `allocation_anchor` 去测配置项;AI-REBALANCE-7 恰好落在「改了还没还原」的窗口里 → 缓存查不到 → 卡片不渲染。**诊断路径值得记**:先确认页面不是登录页(`含登录表单=0`)、大小正常(277KB)→ 排除 session 失效;再对比「qa-run 上下文」与「手工同一时刻」→ 定位到是**上下文差异**而不是代码问题。**判据**:护栏之间有共享状态时,顺序就是隐藏的依赖。一条护栏为了测配置而改配置,必须在**同一条护栏内**还原,不能指望末尾的全局还原 —— 中间那些条已经跑过了。 |
| v1230-MATRIX-LEFT-IN-DOC | **27×6 的指标联动矩阵写进了 PRD,却没有一条断言守它**(2026-09-20 · 维护者质疑后发现)。v1.23 的核心产出是那张矩阵(每个组件在每类操作下该变/不该变),而我的 e2e 只验了「页面渲染了、锚期文字对、库里存了」—— **dashboard 和报表上的数字有没有按矩阵联动,一条都没验**。维护者直接问「你是否亲自在页面上真实完成了双账期并存下几种余额变动后的测试」,答案是没有。**补做之后立刻有收获**:矩阵第 10 行的 F 列标注不准确(见下一条)。**判据**:设计文档里的表格如果不落成可执行断言,它就只是一份「我们当时是这么想的」的记录,没有任何人在守。写完矩阵的下一步是把它变成 flow,不是把它变成 commit。 |
| v1230-MEASURED-THE-WRONG-KPI | **断言选错观测点,两次都指向「功能坏了」的错误结论**(2026-09-20)。验「收益类锚点随填报完成状态变化」时,第一版选了 dashboard 的「紧急储备」—— 它显示成**封顶值「> 36 月」**,几百块的支出根本拉不动;第二版选了「本月资产收益」—— 它的口径是「本月未关账时按已录事实**实时计算**」,锚的是**进行期**,跟收益类锚点根本没关系,两种状态下都是 `-3.21%`。看着都像「判据没生效」,实际是量错了地方。真正的观测点是**报表页的锚期**:没填完显示「数据截至 2026 年 7 月」,填完显示「8 月仍在填报中」—— 一眼分得出。**判据**:选观测点前先读那个指标的 tooltip / 口径说明,确认它**确实由你要验的那个变量驱动**;并且避开被封顶、被四舍五入、被隐私模式糊掉的显示值。量错地方比没量更危险 —— 它会让你去"修"一个没坏的东西。 |
| v1240-INDIRECT-ISOLATION-IS-AN-INFERENCE | **全仓 339 条 SQL 里 156 条没有任何 `family_id` 条件,而且没有一条的方法签名带 `familyId`**(2026-09-20 · v1.24 普查 · 维护者要求)。归属全靠 `period_id` / `account_id` 间接推出来 —— 那是**推论不是约束**:它依赖「账期一定只属于一个家庭」这类当前恰好成立的前提。前提一变(共享账期、合并家庭、修数脚本写错归属),漏隔离的查询会**静默返回别人家的钱**:不报错、不越权告警,只是数字变大。**其中有一条不是「缺条件」而是真 bug**:`BrokerLinkMapper.findAllEnabled()` 查全库 enabled 的券商关联,而唯一调用方 `syncAllEnabled(familyId, …)` 拿到结果后用**自己的 familyId** 逐条 sync —— 定时任务会把别人家的券商账户当成这个家的来同步。**判据**:写任何 `@Select/@Update/@Delete`,先问「这张表是不是家庭作用域」;表上没有 `family_id` 列就 JOIN 到有的表上取,**不接受「上游调用方已经校验过」**(调用方会变,SQL 不会跟着变)。护栏 `v1240-FAMILY-ISOLATION` 逐条扫,例外清单钉在 `scripts/family-isolation-audit.py` 里、新增例外要改两处。 |
| v1240-SELECT-LIST-IS-NOT-A-FILTER | **审计脚本把「SELECT 列表里出现 family_id」当成「有隔离」,漏掉 10 条**(2026-09-20 · v1.24)。第一版判据是「SQL 文本里有没有 `family_id`」,于是 `SELECT id, family_id, … WHERE id = #{id}` 这种**把 family_id 查出来但不拿它过滤**的语句一路绿。收紧成「必须出现在过滤位置(`family_id =` / `IN` / `<`)」之后多抓出 10 条,其中就有 `AccountMapper` / `PeriodMapper` / `GoalMapper` 的 `findById` —— 全站最常用的三个按 id 取实体,一个家庭条件都没有。**判据**:静态判据要判的是**位置**不是**存在**。「这个词出现了」和「这个词起作用了」是两件事,而前者写起来容易得多,所以要警惕。 |
| v1240-COMMAND-GROUP-EXIT-KILLS-THE-RUN | **`{ …; exit 1; }` 是命令组不是子 shell,里面的 exit 把整个 qa-run 退掉**(2026-09-20 · v1.24)。两条 e2e 护栏用它判「每个 flow 都有 X」,其中一条一红,**它后面的所有护栏和最后那行总结一次都没跑过** —— 而终端上看不出异常:没有报错、没有栈,只是最后少了一行「总结: PASS=… FAIL=…」。实测把 `{ }` 改成 `( )` 之后,总结回来了,同时显出了 5 条一直被挡住的红灯。**判据**:判据里带 `exit` 的分组**一律用子 shell `( )`**。更一般地:一个测试套件如果**没有打印它自己的汇总行**,先怀疑它中途死了,而不是「这次刚好没有失败」。 |
| v1240-CONSTANT-BEATS-GREP-FOR-CALIBER | **两条 SQL 口径一致这件事,grep 守不住,常量能**(2026-09-20 · v1.24)。v1.21 的第二层构成与第一层分叉了四处(无家庭隔离 / 不排归档 / 不排调整 / **不换汇**),两条 SQL 各自都跑得出数、都不报错,只是合不上 —— 而 PRD 要求它们相等,按当时的写法**本来就不可能**。grep 护栏能检查「两处写法一样」,但检查不了「将来有人只改一处」。**修**:把过滤块与换汇块提成 `public static final String`,三条查询拼同一个串;Java 注解值接受编译期常量表达式,于是「口径一致」从文档承诺变成**编译期事实**。**判据**:当两处代码必须永远一致时,优先找一个**语言层面的机制**把它们绑在一起(常量、共用方法、生成),其次才是护栏。护栏防的是「有人另写一份」,绑定防的是「有人只改一半」——后者更常见。 |
| v1240-TEMPLATE-MODULO-INDEX-IS-A-TIMEBOMB | **模板里 `catPalette[i.index % 9]`,色板从 9 色改成 6 色之后是渲染期数组越界**(2026-09-20 · v1.24)。编译器看不到、单测覆盖不到、beta 恰好不到 7 个类目所以没炸 —— 它会在**某个类目多的家庭**打开报表时变成 500 白屏(chunked streaming 下连 /error 都不出)。**修**:颜色由服务端按下标逐个给定,超出色板长度显眼地回落成灰;**不取模** —— 取模会让第 7 片和第 1 片同色,那看起来像「它们是一类」,比灰色更误导。**判据**:模板里对数组/列表取模索引,等于把「这个集合的长度」冻成了一个不会被检查的契约。长度一变就是渲染期崩,而渲染期崩在这个项目里是白屏不是错误页。 |
| v1240-SPEC-CONSTRAINT-WAS-UNSATISFIABLE | **PRD 要求「相邻片明度差 ≥ 15」,而它自己给的 6 个品牌色做不到**(2026-09-20 · v1.24)。把 sRGB 真算成 CIE L* 之后:明度分三档(暗 28/30、中 41/42、亮 56/59),中档与暗档最大只差 14、中档两色之间只差 1;6 个色排成一条链,两个中档色各自都需要亮色当邻居,而亮色只有两个 —— **组合上无解**。PRD 自己也意识到了一半(给第 5、6 位开了「色相对立」的例外),所以它真正主张的判据是「明度**或**色相,至少一个拉得开」,只是正文只写了前半句。**处置**:按它实际主张的写护栏(明度差 ≥ 15 **或** 色相差 ≥ 60°),两条都算 —— 仍然是有牙齿的:又同明度又同色相的两个色排一起照样红。**判据**:把规格里的数值约束写成可执行判据时,先验证它**在给定输入下是否可满足**。不可满足的约束只有两种下场:要么护栏永远红被关掉,要么判据被偷偷改宽而没人记得为什么。 |
| v1240-METRIC-NAMED-WRONG-CALIBER | **两个不同口径的数并排放,名字都叫「支出」**(2026-09-21 · v1.24 · 维护者指出)。归因瀑布的数字条写「本期支出」,而它只累加 `category_code='consumption'` 的行;同一块下面的「月均支出」是口径 A(含还贷 / 利息 / 转账给亲属)。两个数相差几十倍,用户拿它们对账必然对不上,然后以为程序算错了 —— 这正是合并前那两个同名 section 让人困惑的同一类问题,我在新块里又造了一遍。**修**:改名「本期日常开支」,并给它、比上期、月均支出、常态月均、刚性占比各配一个 ⓘ(口径文字 + 服务端算好的真实数值串)。**判据**:一个指标的名字必须能把它和同屏的其他指标**区分开**。「支出」在这个产品里有三种口径,任何一个裸的「支出」标签都是错的。 |
| v1240-NEGATIVE-PART-BREAKS-THE-BAR | **某一档退款多于消费时,三分条的百分比算出 103.2% 和 −3.6%**(2026-09-21 · v1.24 · 双端截图审视时发现)。beta 本月有两笔 −499 的数码电器冲正,而数码电器是一次性类目 → 一次性那一档合计为负 → 弹性 / 算术合计 = 103.2%。**三个数学上都对、加起来正好 100%**,但拿去当条宽就是负段画不出来、正段溢出容器。v1.22 在饼图上解决过**完全同一个问题**(合计 ≤ 0 的组不进扇形、但仍进总额),我在三分条上重犯了。**修**:`pct` 保留真值(图例显示,因为它是事实)、新增 `barPct` 按正值合计归一(条宽用它)、`inBar=false` 的档不画段并在图例里标「退款多于消费」。**不许取绝对值** —— 那会把一笔退款画成一笔消费,而且不报错。**判据**:一个项目里解决过的视觉陷阱,换一种图表还会再出现一次。`v1220-NEGATIVE-GROUP-NOT-IN-PIE` 这类护栏要在**新增任何比例图**时主动去找同构的那一条,而不是等它再红一次。 |
| v1240-GUARDRAIL-TOO-BLUNT | **护栏写「不许出现任何 `.abs()`」,立刻把一个合法用法判红**(2026-09-21 · v1.24)。为了守住「不许对分档金额取绝对值」,判据图省事写成扫全文件的 `.abs()` —— 而 `pct()` 里的 `divide(whole.abs(), …)` 是**分母**取绝对值,防的是总额为负时把每一档的符号整体翻过来,那是对的。判据收紧成 `v\.abs\(\)|amountBase\(\)\.abs\(\)`(只盯分档金额)之后才对。**判据**:护栏太钝和太松一样是缺陷 —— 太松漏真错,太钝逼着后来的人去改判据,而改判据的人往往顺手把它改到再也抓不到东西。写「不许出现 X」时先问一句「X 有没有合法的用法」。 |
| v1240-FEATURE-UNVERIFIABLE-ON-BETA | **新做的下钻在 beta 上点不出来,而「点不出来」既可能是设计也可能是 bug**(2026-09-21 · v1.24)。年度累计的细类展开器条件是「细类 > 1」(只有一个细类时展开只会看到一行跟外面一样的,那是骗人点一下),而 beta 锚期窗口内每个大类恰好只有一个细类;唯一有两个细类的组合落在还没定稿的 09 期,报表锚不到它。也就是说**靠在 beta 上点一下来验收这个功能是验不出来的** —— 它不红不绿,只是不出现。**修**:判据落到服务层,用构造出来的数据穷举(`ExpenseYearRollupTest` 5 条)。**判据**:交付一个「条件满足才出现」的 UI 时,先问「beta 的数据能不能让它出现」。不能的话,验收不能只说「我在 beta 上看过了」,必须有一条不依赖现有数据的断言。 |
| v1240-PERCENT-WITHOUT-ITS-BASE | **百分比旁边没有它的基数,而正上方摆着一个口径不同的金额**(2026-09-21 · v1.24 · 维护者两次指出,两处都犯了)。dashboard:「人赚」卡上面一行是「支出 ¥X」(口径 A,含还贷 / 利息 / 转账给亲属),下面我加的三色条拆的是【日常开支】(只算消费),基数比它小 —— 用户拿百分比去乘上面那个数必然对不上。报表页:三分条拆的是【近 12 期】的日常开支(十万量级),而它正上方的数字条写的是【本期】日常开支(百元量级),差一个数量级。**两处的共性**:口径与窗口都只写在 ⓘ 里,没写在脸上。**修**:两块都把「窗口 + 基数」写进标题行(「近 12 期 · 日常开支 ¥X」/「其中日常开支 ¥Y」)。**判据**:一个百分比必须让人**一眼看到它的分母**。ⓘ 是给想深究的人看的,不能拿它承担「防止用户算错」的责任 —— 会去点 ⓘ 的人本来就不会算错。**更强的一条**:同屏出现两个都叫「支出」的金额时,至少有一个名字是错的。 |
| v1240-RESTRAINT-MADE-IT-INVISIBLE | **「不加新组件」守过头,把功能做成了看不见**(2026-09-21 · v1.24 · 维护者连问三次「放哪了」)。PRD FR-654 写着「dashboard 不加任何新组件」,维护者明确推翻了它、要求 dashboard 也有即时的支出分析。我照做了,但仍在迁就那条被推翻的约束:做成「人赚」卡内的一条 **5px 色带、半栏宽、没有标题**,全页 5601px 里落在 y=1424。curl 查得到、截图看得见、e2e 也绿 —— 但维护者打开页面找不到,连问三次。**修**:改成整宽独立块 + `h3` 标题 + 32px 条 + 图例带金额 + 「看近 12 期的完整分析 →」出口。**判据**:约束被推翻之后,要**整体重新权衡**,不能一边执行新要求一边继续照旧约束打折 —— 那样两边都不满足。另:一个功能「做了但找不到」等于没做,而 curl / 单测 / e2e 全都证明不了「找得到」;`h3` 之类的结构性标记是能机器校验的最接近代理(已落 e2e 断言)。 |
| v1240-README-H1-CLOBBERED | **一条没锚定的 sed 把 README 的大标题写坏了,连着两版没人发现,随发版推上了公开仓库**(2026-09-21 · v1.24)。为了把「N 黑盒回归」这个数改掉,用了一条没锚定的替换,结果落到了**文件开头**:第 1 行从 `# 家庭账房 · Family Ledger` 变成 `/ 864 黑盒回归 |# 家庭账房 · Family Ledger`,下一次改这个数又叠成 `||`。**为什么一路绿着**:数测试数的两条护栏都是 `grep -oE '[0-9]+ 黑盒回归' README.md | head -1` —— 而粘进第 1 行的垃圾恰好**就是**第一个匹配,数字还是对的,于是护栏读的是垃圾、报的是绿;顺带把真正过期的那处(第 134 行写着 862、与别处的 865 矛盾)一起遮住了。发现时它已经是 GitHub 首页的 H1。**修**:还原第 1 行;第 134 行的 862→866 与 e2e 数(旧 runner 的「186 断言 / 22 主线」,而 `scripts/e2e.sh` 早在 v1.23 就改造成了 `scripts/e2e/run.cjs`)一并更正;加护栏 `v1240-README-H1-INTACT` 把整行钉死。**判据**:① 改文档里的数字用**锚定过的**替换,改完 `head -1` 看一眼文件开头;② 护栏用 `head -1` / `grep -m1` 取值时,**取到的是不是预期那一处**本身就得有判据 —— 否则它会在读到垃圾时照样变绿,还顺手掩盖别的过期值;③ 没人从第 1 行读自己的 README,所以这类损坏只能靠机器抓。 |
| v1241-MANUAL-DENIED-A-SHIPPED-FEATURE | **站内手册停更十六个版本,而且在主动否认一个已经上线的功能**(2026-09-21 · v1.24 后复盘)。用户点导航栏「手册」看到的 `/help/how-to-use` 最后一次实质更新在 **v1.8**,中间发了 v1.9 → v1.24。光是「没写新功能」还只是难受;真正的问题是它**白纸黑字否认**:「类目停在性质这一层,**不预置「餐饮」这类消费品类**」「类目就这四个(内置)……需要更细的分类可以来提 issue」,而 `docs/how-to-record.md` 里更直接 ——「**Q:我想看支出分类占比?** 目前不支持,支出只到「每月一个总数」这一层,在排期中」。彼时消费分类树已上线数版,beta 上建了 37 个类目,报表第二层就是按消费分类的占比。**后果**:看了手册的人会**停止去找**这个功能 —— 比没写更糟,因为它把「找不到」变成了「不存在」。**为什么一直没发现**:手册是写完就不再读的东西,而所有护栏只检查它的**结构**(章节 id、目录卡数、图片数、入口数),一条都不看**内容是否还成立**。**修**:ch2 改写两个独立分类维度 + 一次性勾;ch3 补关账节奏四档与宽限期双活跃账期;新增 A6 账单批量导入;B8「支出构成」重写为「支出分析」;两份 md 同步;重拍 `ch3-periods.jpg`(旧图是 v1.23 之前的周期页,和新写的文字互相打架)。护栏 `v1241-MANUAL-NO-RETIRED-DENIAL` 把那几句被事实推翻的话钉死。**判据**:① 发功能时,「手册要不要改」和「PRD/QA 要不要改」同级,不是收尾的可选项;② 结构型护栏(有没有这一章)守不住内容型腐烂(这一章说的还对不对)——**能钉的是「退役的话不许再出现」,退役一句钉一句**;③ 手册里的**截图**和正文一样会过期,而图比文字更难发现错 —— 改了某页行为,先搜一遍手册有没有那页的图。 |
| v1242-GUARDRAIL-BLAMED-THE-CODE-FOR-AN-ENV-FAULT | **三条常红护栏,三条都不是代码的问题,而它们的红字全都指着代码**(2026-09-22 · 维护者说「去排查清楚」才查出来)。① `v04-AI-REBALANCE-5` 报「cache 未命中 · log doesn't show fromCache=true」—— 真相是它先 `DELETE` 掉缓存,再指望 LLM 现场生成一条来填,而 beta 的 DeepSeek `status=402`(余额不足)、备选百炼 `status=400`(凭据/权限)双双失效,第一次调用就没产出,自然没有缓存可命中。**缓存代码一直是好的。** ② `v04-AI-REBALANCE-7` 报「刷新按钮缺」—— 同一个根因:那张卡只在缓存里有建议时才渲染。③ `v1230-E2E-TWO-LAYER-ASSERT` 报「有 flow 只验了一层」—— 而被判的 `21-dual-period-metrics` 恰恰是三条 flow 里**页面验得最狠**的一条:它用 `ui.page.evaluate` 从渲染后的 DOM 抠 KPI 值再断言变/不变(22 条 `ui.assert`),比 `seesText` 强得多;判据只认 `ui.seesText|visible|count|sameSize` 四个**名字**,不认这条路。**共同的病**:判据把「第三方账户有没有余额」「用过哪个助手函数」当成了「我们的代码对不对」。**后果比不报警更糟** —— 红字指错方向,每次看到都要重新排查一遍,最后所有人学会忽略这三行,而它们旁边真出问题时也一样被忽略。**修**:①② 改成**自己塞一行缓存**再断言(命中只看 `(family_id, anchor_code)` + 30 天 TTL,不校验 `prompt_hash`,见 `RebalanceAdvisorService:90`),测的是缓存逻辑本身,与 LLM 在不在线无关;③ 把「从 DOM 抠值」并入看得见层,并补一条「必须真的落到 `ui.assert`」(光 `evaluate` 出来不断言、比如只用它滚动定位,不算验)。三条改完成对验证过:只查库的 flow / evaluate 但不断言的 flow / 只验页面的 flow 都仍然红。**判据**:① 一条护栏只该守**我们能修的东西**;依赖外部账户状态的断言必须自带桩,否则它迟早变成一盏没人看的红灯;② 判据认**名字**不认**行为**时,会把更好的做法判成违规 —— 「有没有从渲染结果上取值并断言」才是要守的行为;③ 长期常红的护栏是**技术债**,不是背景噪音:红字写错方向的成本,是每个看到它的人都要重新排查一次。 |
| v1243-REGRESSION-SUITE-WAS-THE-BIGGEST-SPENDER | **账单炸了,查下来最大的花钱方是我自己的回归脚本**(2026-09-22 · 维护者发现阿里云欠费、控制台 3000+ 调用)。`scripts/qa-run.sh` 每跑一次会打 **15 个 AI 端点**(7× `/checkup/diagnose`、5× `/reports/rebalance/advise`、3× `/checkup/insight`),而诊断的缓存是**进程内**的(1h TTL,key 含整个 prompt)—— 重启即失效、股价定时任务一改数字 key 就变,所以基本每次都是 miss。叠加第二件事:**主选 DeepSeek 从 09-02 起就欠费失败**,于是每一次调用都自动 failover 到阿里云百炼,再没有分流。按天对账正好压在 v1.23/v1.24 的开发期上:09-15 一天 **187 次**百炼调用 ≈ 那天跑了十几轮回归。beta 日志里阿里云系累计 **2278 次成功(计费)+ 160 次失败**,而失败正好从 09-20 开始 —— 那天百炼也欠费了。**排除的两个猜想**:① 不是恶意流量 —— beta 密码公开在 README 里,但 14 天 nginx 日志显示打到 AI 端点的只有 `/checkup/diagnose` 8 次、`/checkup/insight` 8 次、`/ask` 约 28 次,MCP 回调审计表里 8 个来源 IP 全是阿里云网段(百炼服务端),53 OK / 11 INVALID(过期 token 被正确拒掉);② 不是 Managed Agent 贵 —— 超级 Agent 全部用量 beta 41 问 + prod 6 问,每问平均 2.1 次工具调用,折算约 200 次模型调用,占不到十分之一。**修**:默认一次真 LLM 都不打 —— 能塞缓存的先塞(命中缓存的请求不出网);塞不了的(诊断走进程内缓存)改成静态判据,或复用 `account=99999` 那条**本来就不碰 LLM** 的早返回路径;真的必须打真 LLM 才有意义的(强制刷新、实调用嗅探、输出合规扫描)默认 skip,要验时 `QA_LLM_LIVE=1`。改完实测**新增 LLM 调用 = 0 次**,护栏 `v1243-QA-RUN-COSTS-NOTHING` 钉住(五种情形成对验过:裸请求判红,先塞缓存/包开关/早返回/注释行判绿)。**判据**:① 一条常规回归护栏**不该花钱** —— 花钱的那种是另一件事,要人主动要才跑;② 这些护栏验的从来不是「LLM 答得对不对」,而是「失败时降不降级、缓存命中没有、按钮渲染没有、跨家庭会不会泄露」,没有一件需要把钱花出去;③ **主备自动 failover 是个静默的成本放大器** —— 主选挂了之后,所有流量会悄悄压到备选上,而没有任何地方会说一声。 |
| v1243-GRACEFUL-DEGRADATION-HID-A-20-DAY-OUTAGE | **每一层都优雅降级,于是 AI 全挂了 20 天没有任何人知道**(2026-09-22)。DeepSeek 最后一次成功是 **09-02**,百炼从 **09-20** 起也报账户级故障 —— 中间 AI 体检、调仓建议、AI 月报、超级 Agent **全部出不来**。而页面上显示的是「AI · 暂不可用」「暂未能生成建议」:那是**正确**的用户体验,但它同时意味着**故障和「这个月没人点过 AI」长得一模一样**。最后是**账单**把这件事捅出来的 —— 那是最差的发现方式:等你看到账单,钱已经花完了,而且第一反应是「是不是被人刷了」。**修**:`LlmHealthTracker` 记每个平台最后一次调用的结果,`LlmRouter`(唯一编排点,成功失败都过那一个 for 循环)负责记录;管理页 `/admin/ai-access` 显示健康读数,并在**所有平台最后一次都是账户级故障**时给出醒目告警,带上距最后一次成功多少小时。**两个刻意的设计**:① 判据收紧到**账户级故障**(401/402/403,不会自己好),偶发超时与单型号额度用尽不报警 —— 否则噪音久了没人看,这块读数会变成又一个「一直红着所以被忽略」的东西;② 读数只在内存里,重启后显示「还没调用过」而**不假装正常** —— 把「不知道」显示成「正常」正是这次事故的形状。**判据**:优雅降级是对的,但它必须配一个**给运维看的通道** —— 否则降级不是在保护用户体验,是在**掩盖故障**。告警那一半在页面上验不出来(要看到它得把 AI 真弄坏),所以判据必须落单测穷举(`LlmHealthTrackerTest` 7 条)。 |
| v1243-TWO-PUBLIC-CTORS-KILLED-STARTUP | **加了个构造器,`mvn package` 绿、901 个单测全绿,应用起不来**(2026-09-22)。给 `LlmRouter` 加健康读数依赖时,为了不改十几处单测,留了一个两参的便捷构造器 —— 于是这个 `@Component` 有了**两个 public 构造器**,Spring 不知道挑哪个,退回去找无参构造,然后 `NoSuchMethodException: LlmRouter.<init>()`,整个应用启动失败。**编译看不见、单测看不见** —— 单测是自己 `new` 出来的,根本不走容器。只有真的启动才看得见。**修**:给生产那个三参构造标 `@Autowired`。**判据**:改 Spring bean 的构造器之后,「编译过 + 单测绿」证明不了它能起来;这个项目已经有一条「curl 200 不等于验过」,这里是它的兄弟 —— **构建绿也不等于起得来**,部署完必须看一眼 `/health`。 |
| v1244-ALL-SELF-TESTS-BYPASSED-THE-REVERSE-PROXY | **我所有的自测都打 127.0.0.1,于是整整漏掉了反代那一层**(2026-09-22 · 维护者要求「不能是 http 接口,是你从端上测」才暴露)。从浏览器在 `beta.dixi-token.top` 上问一个稍复杂的问题(「资产多少 / 分布如何 / 有什么改善建议」),**150 秒一个字都没回来,控制台一个 504**;而同一个问题直连 `127.0.0.1:20000` 是好的。差别只有一个:**nginx**。两件事叠加 —— ① nginx 默认**缓冲**上游响应,SSE 于是根本不是流式,用户盯着空白等;② `proxy_read_timeout` 默认 60s(beta 配的 90s),而应用给 `SseEmitter` 的是 200s,**两边不一致时短的那个说了算**。而这条路径是**所有自建用户的默认路径** —— 仓库发出去的 `deploy/nginx-finance.conf.example` 也是 90s、也没有 SSE 处理。**修**:两道防护都放在**应用侧**(自建用户的反代五花八门,我们控制不了他们怎么配)—— 响应头 `X-Accel-Buffering: no`(nginx 认的标准头,发了就不用改配置)+ 每 15s 一个 SSE 心跳注释(`proxy_read_timeout` 算的是**空闲间隔**,有字节流过就掐不到);另外给 nginx 示例补一个 `/ask/N/stream` 的 location 说明意图。实测:同一个问题从 504 变成 **75~100 秒正常返回,控制台 0 错误**。**判据**:① 「我在本地测过了」对**反代后面的应用**不成立 —— 本项目已有「curl 200 不等于验过」,这是它的第三条兄弟:**直连通不等于端上通**;② 长连接(SSE / WebSocket / 流式下载)必须**假设中间有个会缓冲、会超时的东西**,而且防护要做在自己这一侧;③ 应用超时 > 代理超时时,用户看到的永远是代理那个 —— 两个超时值必须一起想。 |
| v1244-CITE-KEY-COLLISION-SHOWED-THE-WRONG-NUMBER | **正文说「债券理财」,挂上去的卡是「支付宝 48.34%」**(2026-09-22 · 端上实测)。托管 Agent 一轮会调五六次工具,而工具返回的引用 key 是**工具内部的行号**(`r0_0` = 第 0 行第 0 列、`nw` = 净资产)。于是「按资产类型」的 `r0_0` 和「按平台」的 `r0_0` **撞成同一个名字** —— 模型看到的是同一个 key,它没有任何办法区分;我们这边寄存时后到的也把先到的覆盖掉。本机直连那条路一直没事,因为它在循环里重编成 `c1/c2/c3`。**后果比缺数字严重得多**:页面上是一个排版正确、看起来完全合理、实际错位的数,**用户没有任何办法发现它错了**。这正是本项目那条「宁可显示「—」也不给一个看起来合理却没有数学意义的数」要防的东西,只是这次是在 AI 这条路上复发。**修**:在 **MCP 出口**(发给百炼之前)就把 key 重编成 `t<第几次调用>_<原key>` —— 模型拿到互不相同的名字,写回来的标记自然对得上。实测:`t2_r0_0` = 债券理财、`t7_r0_1` = 支付宝,两条并存不再互相覆盖,正文与卡片逐句对上。**判据**:**跨调用聚合别人的 key 之前,先问它在那个别人的作用域里是不是唯一的**。「工具内唯一」和「一轮内唯一」是两个不同的东西,而把前者当后者用不会报错 —— 它只会安静地给错数。 |
| v1244-DELIMITER-COLLIDED-WITH-JSON | **图表标记以 `}}` 结束,JSON 对象也以 `}` 结束,于是模型少打一个括号,整张图静默消失**(2026-09-22)。模型把 `...]}}}}` 写成了 `...]}}}`——拿标记的一个括号当成了 JSON 的收尾。解析失败 → 图被丢掉 → 而正文里「钱主要分在六类里……**:**」那个引导冒号**还留在页面上**,后面空空如也。这不是模型「犯错」:**分隔符和内容的收尾字符撞车,是我们选的格式的问题**,容错就该在我们这边。**修**:`parseChartJson` 解析失败时最多补两个 `}` 再试;真的坏了就判死并**写一行日志**(原来是完全静默的,连查都无从查起)。顺带发现 Jackson 对空串**不抛异常、返回 `MissingNode`** —— 只判 null 会把「什么都没有」当成解析成功,所以判据收紧成「必须是 JSON 对象」。用**同一条已经存在库里的坏标记**做前后对照:改之前 0 张图、改之后 1 张图,PC 与手机都是。**判据**:① 给 LLM 定输出格式时,**分隔符不能和内容里高频出现的字符撞**(`}}` 之于 JSON、`---` 之于 Markdown 都是);撞了就必须容错,而不是指望模型每次都数对;② 解析失败**不许静默** —— 静默失败在页面上的样子是「一个悬空的冒号」,而没有人会因为一个冒号去查日志。 |
| v1246-A-TEMPORARY-FALLBACK-OUTLIVED-FOUR-MONTHS | **体检的风险分布从 v0.2 起就不看产品类目,加密、贵金属、保险全是「无风险」**(2026-09-24 · 处理 issue #23 时读代码查到)。`FamilyDiagnoseService` 里写着「category 暂时拿不到,用账户类型映射……Stage 3.2 会升级」,那个「暂时」留了四个多月。写死的是**字符串 switch + `default -> 0`**:之后加的三个账户类型编译器一声不吭地落进 0 级;没有任何类型到 5 级,`FAM-RISK-1`(高风险 ≥ 40%)从来触发不了;用户在账户页选的类目、手工改的等级,风险分布都不认;报表那张图的副标题还写着「按产品类目风险等级聚合」。**为什么一直没人发现**:图照常画、数照常出,只是分错了档 —— 典型的静默算错。**修**:等级按「手工 → 类目 → 按类型的默认类目」取(`RiskLevels`),默认类目表对枚举穷尽、不留 default;估算的在页面上说出来;报表按等级取色(原来按扇区顺序)。**判据**:① 写「暂时」的兜底必须同时写清楚**什么时候拿掉、谁来拿掉**,否则它就是永久口径;② 对枚举做映射一律用不带 default 的 switch 表达式,让加枚举值的人被编译器拦下;③ 护栏 `v1246-RISK-LEVEL-LIVE` 用「各档合计 = 同页总资产」+「每一档都能在 DB 里解析出来」两个与数据分布无关的不变量,对着修复前的 beta 跑过是红的。 |
| v1246-STALE-TARGET-HID-A-DELETED-TEMPLATE | **报表「钱花在哪了 → 点一行看逐笔」在 Docker 里是整页错误,而我们自己的环境一直是好的**(2026-09-24 · 整套验收时查到)。v1.24 把 `_expense-mix.html` 并进 `_expense-section.html`,控制器还返回旧文件名。beta 和 prod 都在长期存在的工作区里 `mvn package`(不 clean),Maven 不会删 `target/classes` 里源码中已不存在的资源,旧模板一直被打进 jar;Docker 镜像是 `clean package`,没有这个文件。**我们所有的验证都跑在「dirty 构建」上,Docker 用户跑在「clean 构建」上** —— 两边看到的不是同一个程序。这次能抓到,是因为 issue 集成栈是新开的 worktree(clean),同一份代码在两种构建里结果不同。**判据**:① 控制器返回的模板名、模板里的片段引用,护栏 `v1246-VIEW-NAMES-EXIST` 对着文件系统查;② e2e 只断言「这一块渲染出来了」不够,**能点的都要点一下**(新 flow `26-expense-drawer`);③ 验收要跑全三套(e2e / qa-run / regression-data),v1.24.5 发版时 regression-data 主线 15 已经红了 8 条,因为没跑。 |
| v1245-PASTED-THE-GENERATED-URL-BACK-INTO-THE-FIELD-THAT-GENERATES-IT | **管理页展示的 MCP 配置会拼出 `/mcp/mcp`,这是个真 bug,但【不是】2026-09-23 那次生产故障的原因 —— 我一开始把它误判成了原因**(2026-09-23)。那一格问的是**本站公网地址**(站点根),而我们生成给人粘贴的是完整的 `https://域名/mcp`;生产库里那一格被填成了 `https://dixi-token.top/mcp`,于是管理页展示的配置变成 `/mcp/mcp`(该路径被普通 Web 安全链接管,302 跳登录)。我据此下了结论,维护者贴出百炼里**实际填的**配置 —— `https://dixi-token.top/mcp`,是对的 —— 结论被推翻。真因见 `v1246-REMOTE-AGENT-TEMPLATE-NEVER-UPGRADED`。**仍然修了它**:`normalizeBaseUrl` 剥掉末尾的 `/mcp`,保存与生成两处都归一,只吃末尾那一段(域名里带 mcp、`/mcp-gateway` 子路径不动)。**判据**:① 「本地配置里有个错」和「这个错造成了故障」是两件事 —— 要证明因果,得看到**那个错的值真的用在了故障路径上**(这里要问的是:百炼实际拿到的 URL 是哪个?答案在维护者手里,不在我们库里);② 我当时其实有一条更硬的证据没先看:nginx 日志 14 天里**零条**来自阿里云的 `/mcp` 请求 —— 如果百炼在打 `/mcp/mcp`,日志里会有 302。**零条**说明它根本没来,问题在百炼那侧。**先看「请求有没有来」,再看「来了之后错在哪」**。 |
| v1246-REMOTE-AGENT-TEMPLATE-NEVER-UPGRADED | **生产的超级 Agent 从 09-04 起就查不到任何数据,因为百炼上那个 Agent 模板从没更新过 —— 发新版本不会动它**(2026-09-23 · 维护者报「prod 还是不行」,agent 原话「目前这个会话里没有任何工具连接到我这边」)。时间线:**09-04** 生产的 Agent 在百炼上创建,当时的代码只写了 `mcp_servers`(声明服务器)、没写 `tools`(启用工具);**09-08** 这个 bug 修了(`24d29f3`),**09-09** 随 v1.20.2 发布 —— **但 Agent 是远端资源,新代码只在有人点「更新百炼上的 Agent 模板」时才会写过去**,生产那边从来没人点过。从百炼读回来的两份定义放在一起就是答案:`生产 tools = []` / `beta tools = [mcp_toolkit …]`。**为什么这么难查**:① agent 那句话听起来像**用户自己的 MCP 配置没生效**,把人往「检查百炼控制台」上引;② 我们这边**一条错误都不会记录** —— 百炼根本没来调,入站审计表与 nginx 日志里 14 天**零条**来自阿里云的 `/mcp` 请求(这一条才是最硬的证据,我一开始没先看它,绕去查了 URL,见 v1245);③ beta 一直是好的,因为 beta 的模板被更新过 —— **「beta 好 ≠ prod 好」在远端资源上格外成立**,两边的代码一样,远端状态不一样。**修**:① `templateDrift()` 去百炼读回 Agent 定义,对比「启用了我们哪些工具」(管理接口,不调模型、不花钱);② 提问前先查(5 分钟缓存,更新后立即作废):一个工具都没启用 → 直接给用户**说得出下一步**的提示「模板是旧版本,去点更新」,不让 agent 去编那句话;③ 管理页出醒目横幅 + 就地一个更新按钮;④ **旧会话自动换新**:百炼的会话**锁定创建时的模板版本**,只更新模板不够 —— 之前开的对话仍然停在坏模板上,用户点完更新回到原对话一问,还是「没有工具」,只会以为更新没生效。会话引用改成 `sesn_xxx@v<版本>`,版本对不上就新开(老格式视为版本未知,新开一次;代价是那些对话在百炼那边的上下文记忆断一次)。**验收**(beta 上把模板原样弄成生产那样再走全流程,真实域名、浏览器操作):模板过期 → 横幅出现 → 提问得到「模板旧」提示而非「没有工具」→ 点横幅按钮 → 横幅消失 → 新对话当下 36 引用块 1 图 → **更新前开的旧对话**当下 30 引用块。顺带发现:百炼的 Agent 更新**不是全量替换** —— 请求里不带 `tools` 时它**保留原值**,要显式传 `tools: []` 才清空。**判据**:① **凡是「代码写一次、远端就定格」的资源(第三方 Agent 模板、webhook 注册、云函数配置),发版都不会自动更新它** —— 要么发版流程里有一步去对齐,要么运行时能检测出「远端落后于代码」;② 检测到了就**在用户能看到的地方说清下一步**,不要让中间那个 LLM 去转述一个它自己也不理解的基础设施故障;③ 排查外部集成时,**先确认请求到底有没有来**(nginx / 入站审计),再去猜来了之后错在哪。 |
| v1246-LIVE-STREAM-NEVER-GOT-THE-CITES | **引用落了库,却从没送到正在看的浏览器 —— 流式回答的当下每个数字都是空的,刷新才出来**(2026-09-23 · 端到端验收时发现)。v1.24.3 修了「托管模式下引用不落库」,验收时我看到 147 个引用块 —— **那是刷新页面之后从库里读出来的**。前端在流式过程中靠 SSE 的 `cite` 事件把 `{{cite:…}}` 替换成数字,而这个事件只在 `toolDone(…, citable)` 里发;托管模式下工具是百炼经 MCP 调的,`citable` 永远是空的。于是用户在**那个当下**看到的是「总资产、负债和净资产:」后面什么都没有 —— 而维护者在生产上看到的正是那个当下。**修**:`AskSink` 加一个只推引用的口子;Collector 在每段文字到达之前,把寄存器里**新到的**引用先推给浏览器(时序天然成立:模型要先拿到工具结果才写得出引用标记,而工具结果就是在 MCP 请求里寄存的)。验收改成**在流式回答的当下数**「最后一轮」里的引用块,不刷新。**判据**:流式 UI 有**两个渲染时刻** —— 实时拼出来的那一次,和刷新后从库里读出来的那一次 —— 两条是**不同的代码路径**,一条对不代表另一条对。**验收必须在用户第一眼看到的那个时刻做**,刷新后的样子只是第二眼。 |
| v1246-BUTTON-CALLED-A-FUNCTION-THAT-NEVER-EXISTED | **体检「值得做的事」的「不适用」按钮,从加上那天起就没工作过**(2026-09-23 · issue #22 · 用户报「按下没有反应」)。v0.2(2026-05-10)的提醒卡片上写的是 `onclick="advice.dismiss(ruleId, accountId)"` —— 一个**从没写过的**前端函数,后端也从来没有对应的接口。点击时浏览器报 `advice is not defined`,页面上什么都不发生。**四个多月没人发现**,因为一个点了没反应的按钮不会报错给任何人看:服务端日志里没有它,因为请求根本没发出去;所有护栏都在检查「按钮存在」,没有一条检查「点了之后发生了什么」。**修**:① 按钮改成普通表单提交(不依赖任何脚本),接口 `POST /checkup/advice/dismiss`;② 「不适用」按「规则 + 账户」存进家庭级配置 —— 只在前端藏的话,刷新一下就回来了,家里另一个人在别的设备上也照样看得到,等于没点;③ 家庭体检、账户体检、AI 诊断三处都过滤(用户说了不适用,就不该再拿它去问 AI);④ 提供「已隐藏 N 条 · 全部恢复」,点错能撤回;全部标完时整块区域不能跟着消失,否则恢复入口也没了。没有加表,放在已有的家庭配置里。**判据**:① 模板里的 `onclick="xxx.yyy(…)"` 是编译器和单测都看不见的调用 —— 加按钮时必须同时确认处理函数存在;② 「按钮存在」不是验收,**点一下看结果**才是 —— 这条的护栏 `v1246-ADVICE-DISMISS-LIVE` 真的去点、去看它消失、再点恢复看它回来;③ 端上验收时又抓到一处:传一个**不是自己家的**账户号,原来被当成「没传账户」存成了家庭级记录 —— 一个针对账户的操作悄悄变成对全家生效。改为直接忽略,护栏 `v1246-ADVICE-DISMISS-FOREIGN-ACCOUNT` 真的发一个别人家的账户号过去,确认什么都没存。 |
| v1245-ONE-FEATURE-CONFIG-SPLIT-ACROSS-TWO-PAGES | **超级 Agent 要用的配置分在两个管理页,排查一个问题要来回切**(2026-09-23 · 维护者:「AI 接入为什么是独立的 tab?」)。百炼的 API Key、型号在「数据源接入」,Agent / MCP 服务 / 口令在「AI 接入」;代码里的错误提示也在两页之间互相指。同一天的实例:我把「AI 健康」读数加在了 AI 接入页,而它检查的那几把 Key 在数据源接入页 —— 我自己也被这个分法带偏了。**为什么当初会这样分**:大模型 Key 从 v0.x 就在数据源接入(那时 AI 只是「另一个外部数据源」);v1.19 做超级 Agent 时为 MCP 口令新开了一页,没回头看旧的那一节该归谁。**修**(维护者选 B):大模型那一节整体挪到 AI 接入,端点跟着搬到 `/admin/ai-access/llm/*`(页面挪了而后端还叫 integrations,下一个排查的人会被带偏);数据源接入只留行情 / 汇率 / 贵金属 / 券商 / 宏观;侧栏两项相邻,并去掉 AI 接入后面误导的「· 只读」。护栏 `v1245-AI-SETTINGS-ONE-PLACE`。**顺带查出的漏洞**:入口登记表(`scripts/entry-points.json`,由真浏览器逐条验证入口可见)里**根本没有「AI 接入」**,于是这一页的入口从来没被护栏守过。**判据**:① 一项功能的全部配置应该在**一处**配齐,「按技术层次分页」(这是数据源 / 那是接入)对用户没有意义,用户是按「我要让 X 能用」来找的;② 加新页面时要回头问一句「旧页面里有没有东西其实属于这一页」;③ 新页面上线要同时进入口登记表,否则它的可见性没人守。 |
| v1246-CROSS-CURRENCY-TRANSFER-IN-AMOUNT | **跨币种划转后,美元账户冒出「900 未解释」**(2026-09-23 · issue #21 · 用户报)。人民币账户 A 转 1000 到美元账户 B,B 实到 100 美元(汇率 1:10)。两边余额都更新对了,B 的账户行却提示「余额出现未解释变化」,差额 900。**根因**:填报页给 B 算「本期已知流入」时加的是 `transfer.getAmount()` —— 那是转出方付出的钱(1000,人民币),不是 B 收到的 `to_amount`(100,美元);于是 100 − 1000 = −900。同一处的转入明细也用错了,还套上了 B 的币种符号,显示成「+$1,000」。**为什么这么久没被发现**:`to_amount` 从 v0.8 就有了,新增划转、撤销划转两处都用对了;事实表(`FactMapper.xml`)用的是 `COALESCE(to_amount, amount)`,所以收益率、净资产一直是对的 —— 只有填报页的对账和明细在手写 `getAmount()`,而跨币种划转本身不常见。**修**:`Transfer.receivedAmount()`(站在转入方取金额,只许走这里),EntryService 三处(差额、明细、撤销)都改用它。**端上验收时又抓到一处**:删除这笔划转时的确认框,两边用的是同一个数、币种写死成 ¥ —— 从人民币账户那行点删除写「test银行卡 −¥1,000」,从美元账户那行点写「备用金 +¥100」,两边都说错了(实际执行的撤销是对的,但用户是看着确认框决定删不删的)。流水条目加 `counterAmountLabel`(对方那一侧的金额,按对方币种格式化),确认框两边各用各的。**顺带**:全量导出的 `transfers.csv` 里根本没有 `to_amount` 列,跨币种划转导出后「对方实到多少」丢了 —— 补在最后一列,不动已有列的位置。**判据**:一个实体有两个同类数值字段(付出 / 收到)时,「站在哪一方就用哪个」这个判断要收成实体上的方法,而不是每个调用点各自写三元表达式 —— 后者总有一处会写成默认字段。 |
