...
| v1246-BUTTON-CALLED-A-FUNCTION-THAT-NEVER-EXISTED | **体检「值得做的事」的「不适用」按钮,从加上那天起就没工作过**(2026-09-23 · issue #22 · 用户报「按下没有反应」)。v0.2(2026-05-10)的提醒卡片上写的是 `onclick="advice.dismiss(ruleId, accountId)"` —— 一个**从没写过的**前端函数,后端也从来没有对应的接口。点击时浏览器报 `advice is not defined`,页面上什么都不发生。**四个多月没人发现**,因为一个点了没反应的按钮不会报错给任何人看:服务端日志里没有它,因为请求根本没发出去;所有护栏都在检查「按钮存在」,没有一条检查「点了之后发生了什么」。**修**:① 按钮改成普通表单提交(不依赖任何脚本),接口 `POST /checkup/advice/dismiss`;② 「不适用」按「规则 + 账户」存进家庭级配置 —— 只在前端藏的话,刷新一下就回来了,家里另一个人在别的设备上也照样看得到,等于没点;③ 家庭体检、账户体检、AI 诊断三处都过滤(用户说了不适用,就不该再拿它去问 AI);④ 提供「已隐藏 N 条 · 全部恢复」,点错能撤回;全部标完时整块区域不能跟着消失,否则恢复入口也没了。没有加表,放在已有的家庭配置里。**判据**:① 模板里的 `onclick="xxx.yyy(…)"` 是编译器和单测都看不见的调用 —— 加按钮时必须同时确认处理函数存在;② 「按钮存在」不是验收,**点一下看结果**才是 —— 这条的护栏 `v1246-ADVICE-DISMISS-LIVE` 真的去点、去看它消失、再点恢复看它回来。 |
