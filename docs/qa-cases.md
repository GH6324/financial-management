...
| v19-UPD-BADGE | 版本徽记可点 + 圆点绝对定位 + **不得嵌套 `<a>`**(2026-08-05 · v1.9 FR-301 · 维护者拍板放徽记而非管理 tab)。施工时撞到:徽记原先嵌在 logo 的 `<a th:href="@{/}">` **里面**,直接给它加 href 就成了 `<a>` 套 `<a>` —— 非法 HTML,浏览器会自动拆开、布局散架。改法:把 nav 拆成三个并列 `<a>`(图标 / 品牌行 / 徽记),视觉位置一像素不动(实测 img·brand·ver 的 top/left/height 与改动前逐项相同,navH 38.3、tabs top 15.5 全同)。断言:`ver-badge` + `ver-dot` + 跳 `admin?tab=version`;`.ver-dot` 绝对定位;管理页有 `id="version"` 锚点。 |
