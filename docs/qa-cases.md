...
| v111-RATIO-ABSURD | 比率类分母过小时不许显示荒谬数字(2026-08-12 · v1.11 · prod 实测)。prod 收支稀疏(PMC 仅 6 行):某期收入 300 / 支出 7450 → 储蓄率 **−2383%**。数学没错但毫无信息量,反而像系统算错了。绝对值 > 500% 时显示「收支不足」,原值放 tooltip —— 这不是隐藏问题,是把「分母太小导致比率失真」说出来。 |
