...
| v164-CHART-PARITY | dashboard 两图窄屏形态一致(2026-07-27 · v1.6.4 · 用户反馈④)。「资产配置」与「按成员分布」此前一个环图一个条形,手机上并列看着不像一套东西。抽共用工厂 `hBarConfig(labels, values, total, title, palette)`,窄屏(<640px)两者一律横向条形(窄屏环图标签必然重叠);PC 保持「>6 类条形、≤6 类环图」规则。顺带消除了两处几乎重复的 bar 配置实现。守护断言两图的 `memFlat`/`flatAlloc` 均含窄屏判定且都走 hBarConfig。守护 v164-CHART-PARITY |
