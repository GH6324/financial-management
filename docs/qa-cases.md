...
| v1245-PASTED-THE-GENERATED-URL-BACK-INTO-THE-FIELD-THAT-GENERATES-IT | **生产上超级 Agent 说「没有任何工具连接到我这边」,而我们这边一条错误记录都没有**(2026-09-23 · 维护者报「prod 还是不行」)。管理页那一格问的是**本站公网地址**(站点根),而我们生成给用户去百炼控制台粘贴的却是一条**完整的 `https://域名/mcp`** —— 于是那条完整地址又被填回了这一格。代码是 `baseUrl + "/mcp"`,拼出来就是 **`/mcp/mcp`**。**为什么这个错特别难查**:① `/mcp/mcp` 不是我们的端点,它被**普通 Web 安全链接管**,回 **302 跳登录页**;百炼拿到一个 HTML 跳转,得出的结论是「这个服务器没有工具」,然后 agent 一本正经地把这个结论讲给用户听 —— 听起来像是**用户自己的 MCP 配置没生效**,而不是我们拼错了 URL;② 请求**根本没走到鉴权那一步**,所以入站审计表里连一条失败都没有,只能看出「百炼从 09-03 起就不来了」;③ 正确的 `/mcp` 在凭据不对时回 **404**(McpEndpoint 的「未通过一律 404」),和 302 长得完全不一样 —— 但没人会去比这两个状态码。**定位方法**(可复用):拿**一个肯定不存在的路径**做对照 —— 不存在的路径 302、`/mcp` 404、`/mcp/mcp` 302,于是「`/mcp/mcp` 和不存在的路径行为完全一致」这件事就成了铁证。**修**:`normalizeBaseUrl` 把末尾的 `/mcp` 反复剥掉(粘两次也认),保存时与生成时两处都归一;边界钉死在「只吃末尾那一段」—— 域名里带 mcp、或部署在 `/mcp-gateway` 子路径下的不许动,否则归一化会从「修错别字」变成「改坏别人的正确配置」。**判据**:**当一个输入框的值会被我们拼成另一个字符串、而那个字符串又会展示给用户去别处粘贴时,用户把它粘回来只是时间问题** —— 这种回环歧义必须在代码里消化掉,不能写进文档里指望别人不踩。另:**LLM 会把基础设施故障转述成一句听起来像用户配置问题的话**,所以「agent 说 X」永远不能当诊断结论用,要去查我们自己的入站记录。 |
