...
| **月初任意一天**(需先配关账宽限) | **流程 1 · 填报** | ~10 分钟 | 收入直接入账户、支出汇总到家庭、持仓自动估值 |
...
> **什么时候填 —— 先把这件事配好(v1.23 起)**
>
> 微信 / 支付宝 / 信用卡最方便拿到的是「**整月支出**」,而那个数要等月底过完才有。
> 系统默认在次月 1 日凌晨就关上月账期(**T+0**),关了就改不了 ——
> 所以「月初拿完整账单再填」这条路,**需要先在管理页配一个关账宽限**:
>
> `系统设置 → 周期管理 → 账期什么时候关` · 选 **月底过完再给 2 天(T+2)** 或 T+5。
>
> 配好之后,月初打开填报页会看到**两期并排**:上个月标着「还能改 · X 月 X 日关账」,
> 这个月标着「进行中」。默认落在上个月 —— 宽限那几天里你多半就是来填它的。
>
> 代价要知道:上月的报表、AI 月报、指标也会跟着晚 N 天出。
> 不配就是 T+0,行为与 v1.22 及以前完全一致(那就按「月末倒数两天」填)。
...
默认是**总额**:每位家庭成员各有一个「本月总支出(一个数)」的框,从微信 / 支付宝 / 信用卡账单抄一个总数进去就行,**不用拆分类、不用逐笔**。10 分钟那条路径靠的就是这个。

想看「钱具体花在哪」,在 `管理 → 提醒与模板 → 支出录入方式` 切到**逐笔**。逐笔要多花时间,换来的是整章**支出分析**(见下方[路径 F](#路径-f--这个月为什么多花了砍不砍得掉))。
嫌一笔笔敲慢,`/expense/import` 可以直接导入支付宝 / 微信账单,一次几百笔。**切换不动任何已录数据**,想切回去随时切。

逐笔支出有两个独立的分类:**资金性质**(日常开支 / 还贷 / 利息 / 给亲属,内置)和**消费分类**(餐饮、交通……你自己维护的树,可选)。另有一个「这笔是一次性的」勾,用来保护「常态月均」那个数。
...
| 这个月**为什么多花了**、这些钱**砍不砍得掉** | 报表 ·「钱花在哪了」(见下方[路径 F](#路径-f--这个月为什么多花了砍不砍得掉))· 需先切逐笔 |
...
## 路径 F · 这个月为什么多花了,砍不砍得掉

> **前提**:支出录入方式要切到**逐笔**(`管理 → 提醒与模板`)。总额模式只有一个数,拆不出构成 —— 那种情况下这一章整章不出现,不会塞给你一堆空图。

**报表 →「钱花在哪了」**,从粗到细三步:

**① 钱花在哪** —— 两层,不是两块。第一层按**资金性质**(日常开支 / 还贷 / 利息 / 给亲属),这一层的合计**就是家庭支出总额**;第二层展开其中的「日常开支」,按你自己维护的**消费分类**看。两层都能切「按类目 / 按账户 / 按成员」,点任意一格能展开那一格的逐笔明细。

**② 为什么变了** —— 归因瀑布把两期的差额**摊到类目上**。读的时候记住:**金额差是主信息,变化率是次信息** ——「旅游多了一笔」比「餐饮涨了 100%」更值得看。

**③ 砍不砍得掉** —— 日常开支按「少了它日子过不过得下去」拆成**刚性 / 弹性 / 一次性**三段。性质是**消费分类的属性**,在 `/expense/categories` 一屏改完,改完立即对全部历史生效;单笔要破例就在录入时勾「这笔是一次性的」,逐笔的勾优先。

配套的一个数:**常态月均**。月均支出把买冰箱那个月也算了进去,常态月均剔掉一次性的部分 —— 算「应急储备够撑几个月」时,后者才是你要的分母。

**不想等关账**:仪表盘上有一份**本月至今**的即时口径。注意它和报表那一章**窗口不同、基数差一个数量级**,两处都把窗口和基数写在标题旁边,别交叉相乘。

这一章**不给好坏评价** —— 花钱没有对错,占比高只说明可压缩空间小。不需要的话,`管理 → 计算口径` 一个开关关掉整章(别处引用的「常态月均」会一起消失,不留半开状态)。
