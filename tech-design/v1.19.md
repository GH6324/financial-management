...
### 选型二 · 用哪个 runtime(**已定:Managed Agents**)

| | **Managed Agents(选定)** | Responses API 内联 MCP |
|---|---|---|
| 用户接入步数 | 6 步(3 步人工:两次复制粘贴 + 一次进百炼控制台) | 1 步 |
| runtime 能力 | **session 服务端持久化 · 中断续接 · 多步工具编排 · skill 生态** | 多轮靠 `previous_response_id` |
| agent loop | 不用我们写 | 也不用我们写 |
| 凭据暴露路径 | 经用户剪贴板 | 不经人手 |

**选定 Managed Agents**,理由是维护者要的 runtime 能力 —— session 持久化与中断续接是
