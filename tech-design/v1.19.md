...
### 选型二 · 用哪个 runtime(**已定:Managed Agents · 另配本机直连兜底**)

| | **Managed Agents(选定)** | **本机工具循环(兜底)** | Responses API 内联 MCP |
|---|---|---|---|
...
> **实测边界**:本机直连已在 beta 端到端跑通(见 §四.4)。托管路线的云端往返
> **尚未在真实环境验证** —— beta 只有 IP、没有域名和证书,百炼回调不到。
> 代码按已查证的接口形态实现,但在拿到一个有公网 HTTPS 的环境跑通之前,
> 不要在任何地方把它描述成已验证。
