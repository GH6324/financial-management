...
| # | 待测项 | 不通过怎么办 |
|---|---|---|
| T1 | 百炼控制台注册自定义 MCP 时,**能否配置 `Authorization` 头** | 不能 → **不做 MA 路线**(绝不退化成 token 放 URL —— 违反 MCP 规范且必进日志) |
| T2 | Responses API 的 `tools[].headers` **实际是否透传** | 不能 → 同上 |
...
> **`AgentRuntime` 抽象的取舍**:第一版只有一个实现,抽象是**投机性**的。
> 保留的唯一理由:百炼 Assistant API 三个月前还是官方推荐、现在已「下线中」——
> 这一领域的接口寿命**短于本项目的版本节奏**,而业务层里有工具分发与口径引用,
> 那是最不该被供应商变更牵动的部分。若评审认为过度设计,可拿掉,代价是换接口要动 `AskConversation`。
...
| | 批量上限 | 单次响应行数**上限 500**,超出截断并在 `meta` 里**显式标注**(不静默) |
...
| **第二刀** | 产品内「问一问」:`AgentRuntime` + **引用标记与渲染** + SSE + 两种壳 + 会话存储 + 预置问题 + 新对话 | 非技术用户点预置问题即可用 |
...
### 选型二 · 用哪个 runtime(**待维护者裁定**)

| | **Managed Agents** | **Responses API 内联 MCP** |
|---|---|---|
| 用户接入步数 | **6 步**(3 步人工:两次复制粘贴 + 一次进百炼控制台) | **1 步**(点「启用」);凭据不必显示给用户 |
| 卡点 | `mcp_servers` 只能引用已注册服务,而**注册无公开 API** | 无 |
| agent loop | 不用我们写 | **也不用我们写**(百炼去调 MCP) |
| runtime 能力 | session 服务端持久化 · 中断续接 · 沙箱 · skill | 多轮靠 `previous_response_id` |
| 凭据暴露路径 | 经用户剪贴板,可能被截图 | **不经人手** |
| 鉴权头支持 | **待实测 T1** | **待实测 T2** |

**建议**:先做 Responses 内联(一键),Managed Agents 作为「高级接入」加一个入口。
两条**共用同一个 `/mcp` 端点、同一套凭据体系、同一份 `AskToolRegistry`** —— 不是两套实现。

**若裁定只走 Managed Agents**:接受 6 步接入,并在部署文档里把那 3 步人工操作写成图文步骤。
