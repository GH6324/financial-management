...
- **Pointer 事件**统一手机长按 + 桌面按住;`pointerdown` 命中 `[data-priv]` → 加 `priv-peek` class;`pointerup/cancel/move 离开/scroll/window blur` → 移除。
- 仅隐私态生效(非隐私态 `pointerdown` 直接 return)。触摸时 `preventDefault` + CSS `-webkit-touch-callout:none` + `user-select:none` → 阻止长按选中与呼出菜单。
- **CSS 覆盖**:`html.privacy [data-priv].priv-peek{ filter:none }` 比 `html.privacy [data-priv]` 多一个 class → 特异性更高,**无需 !important**。
- 可发现性:隐私态 `[data-priv]` 显示 `cursor:pointer` + 浮动 chip 文案「长按可看」。
- 只显示被按住的那一个(其余仍遮)。
